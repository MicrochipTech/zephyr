...
#ifdef CONFIG_PM
/* PM policy device contraints calls pm_policy_state_lock_get/put for the
 * PM staties specified in the instances DT node zephyr,disabling-power-states property.
 * pm_policy_device_power_lock_get/put is a do nothing if CONFIG_PM_POLICY_DEVICE_CONTRAINTS
 * is not defined.
 * If CONFIG_PM_DEVICE is defined then pm_device_busy_set/clear are available.
 * These routines use atomic_set/clear_bit in the instances struct device * dev->pm.flags member.
 * A one-bit flag will not work in this driver because we have both TX and RX events.
 */
static void uart_mec5_pm_policy_state_lock_get(const struct device *dev,
					       enum uart_mec5_pm_policy_state_flag flag)
{
...
static void uart_mec5_pm_policy_state_lock_put(const struct device *dev,
					       enum uart_mec5_pm_policy_state_flag flag)
{
#ifdef CONFIG_PM_POLICY_DEVICE_CONSTRAINTS
	ARG_UNUSED(flag);
	pm_policy_device_power_lock_put(dev); /* implemented as atomic_dec */
#else
	struct uart_mec5_dev_data *const data = dev->data;
...
static int uart_mec5_configure(const struct device *dev, const struct uart_config *cfg)
{
	...
	struct mec_uart_regs *base = dev_cfg->base;
	int ret = 0;
	uint32_t mcfg = 0;
	...
	ret = mec_hal_uart_init(base, cfg->baudrate, mcfg, 0);
	if (ret != MEC_RET_OK) {
		return -EIO;
	}
				   ...
				   struct gpio_callback *cb, uint32_t pins)
{
	...
	struct uart_mec5_dev_data *data = CONTAINER_OF(cb, struct uart_mec5_dev_data, rx_wake_cb);
	const struct device *dev = data->dev;

	k_timeout_t delay = K_MSEC(CONFIG_UART_CONSOLE_INPUT_EXPIRED_TIMEOUT);

	uart_mec5_pm_policy_state_lock_get(dev, UART_MEC5_PM_POLICY_STATE_RX_FLAG);
...
static void uart_mec5_rx_refresh_timeout(struct k_work *work)
{
	...
	struct uart_mec5_dev_data *data =
		CONTAINER_OF(dwork, struct uart_mec5_dev_data, rx_refresh_timeout_work);
	const struct device *dev = data->dev;

	uart_mec5_pm_policy_state_lock_put(dev, UART_MEC5_PM_POLICY_STATE_RX_FLAG);
}
#endif

static int uart_mec5_pm_action(const struct device *dev, enum pm_device_action action)
{
	const struct uart_mec5_device_config * const devcfg = dev->config;
	struct mec_uart_regs *regs = devcfg->base;
	int ret = 0;

	switch (action) {
	case PM_DEVICE_ACTION_RESUME:
		mec_hal_uart_activate(regs, 1); /* ungate UART clocks */
		break;
	case PM_DEVICE_ACTION_SUSPEND: /* Enable UART wake interrupt */
		/* NOTE: clearing UART activate clock gates the UART but causes TX to go low */
		mec_hal_uart_activate(regs, 0);
		if ((devcfg->wakeup_source) && (devcfg->wakerx_gpio.port != NULL)) {
...
static int uart_mec5_init(const struct device *dev)
{
	...
	ret = uart_mec5_configure(dev, &data->ucfg);
	if (ret != 0) {
		return ret;
	}
...
#ifdef CONFIG_PM_DEVICE
#ifdef CONFIG_UART_CONSOLE_INPUT_EXPIRED
		k_work_init_delayable(&data->rx_refresh_timeout_work, uart_mec5_rx_refresh_timeout);
#endif
	if ((devcfg->wakeup_source) && (devcfg->wakerx_gpio.port != NULL)) {
		gpio_init_callback(&data->rx_wake_cb, uart_mec5_wake_handler,
				   BIT(devcfg->wakerx_gpio.pin));

		ret = gpio_add_callback(devcfg->wakerx_gpio.port, &data->rx_wake_cb);
		if (ret < 0) {
...
static int uart_mec5_poll_in(const struct device *dev, unsigned char *c)
{
	...
	ret = mec_hal_uart_rx_byte(base, (uint8_t *)c);
	if (ret == MEC_RET_OK) {
		ret = 0;
	} else {
		ret = -1;
...
static int uart_mec5_err_check(const struct device *dev)
{
	...
	mec_hal_uart_raw_status(base, MEC_UART_STS_REG_LINE, &lsr);
	lsr &= (MEC_UART_LSR_OVR_ERR_Msk | MEC_UART_LSR_PAR_ERR_Msk
		| MEC_UART_LSR_FR_ERR_Msk | MEC_UART_LSR_BREAK_Msk);
	ret = (int)(lsr >> 1);
...
static int uart_mec5_irq_update(const struct device *dev)
{
	const struct uart_mec5_device_config * const dev_cfg = dev->config;
	struct uart_mec5_dev_data *dev_data = dev->data;
	struct mec_uart_regs *base = dev_cfg->regs;
	k_spinlock_key_t key = k_spin_lock(&dev_data->lock);
...
#define UART_MEC5_DEVICE_INIT(n)					\
	...
	static const struct uart_mec5_device_config uart_mec5_dev_cfg_##n = { \
		.base = (struct mec_uart_regs *)(DT_INST_REG_ADDR(n)),	\
