...
static void set_baud_rate(const struct device *dev, uint32_t baud_rate)
{
	...
	struct uart_regs * const regs = dev_cfg->regs;
...
static int uart_xec_init(const struct device *dev)
{
	...
	ret = uart_xec_configure(dev, &data->uart_config);
	if (ret != 0) {
		return ret;
	}
...
static int uart_xec_irq_tx_ready(const struct device *dev)
{
	...
	int ret = ((data->iir_cache & IIR_ID) == IIR_THRE) ? 1 : 0;
...
static int uart_xec_irq_rx_ready(const struct device *dev)
{
	...
	int ret = ((data->iir_cache & IIR_ID) == IIR_RBRF) ? 1 : 0;
...
static int uart_xec_irq_is_pending(const struct device *dev)
{
	...
	int ret = (!(data->iir_cache & IIR_NIP)) ? 1 : 0;
...
{
	const struct uart_xec_device_config * const cfg = dev->config;
	struct uart_xec_dev_data * const data = dev->data;
	...
	mchp_xec_ecia_girq_src_clr(cfg->girq_id, cfg->girq_pos);
}
