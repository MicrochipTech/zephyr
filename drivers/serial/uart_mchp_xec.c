...
static void set_baud_rate(const struct device *dev, uint32_t baud_rate)
{
	...
	if ((baud_rate != 0U) && (dev_cfg->sys_clk_freq != 0U)) {
		...
		sys_write8((uint8_t)(divisor & 0xff), ub + XEC_UART_BRGD_LSB_OFS);
		/* bit[7]=0 1.8MHz clock source, =1 48MHz clock source */
		sys_write8((uint8_t)((divisor >> 8) & 0x7f), ub + XEC_UART_BRGD_MSB_OFS);
 ...
 *  an interupt. RX data is available and RX data is available but no bytes
