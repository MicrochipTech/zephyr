...
#if defined(CONFIG_MSPI_XEC_HANDLE_FIFOS_IN_SYSTEM_WORKQUEUE)
	struct k_work fifo_work;
	uint32_t imr;
#endif
};

struct mspi_xec_drvcfg {
	mm_reg_t qbase;
	uint16_t enc_pcr;
	uint8_t girq;
	uint8_t girq_pos;
...
static void mspi_xec_isrconst struct device *dev)
{
#if defined(CONFIG_MSPI_DMA)
	struct mspi_dw_data *dev_data = dev->data;
...
static int api_mspi_xec_config(const struct mspi_dt_spec *spec)
{
	ARG_UNUSED(spec);

	/* TODO MCHP do we need to implement this, other drivers do */
	return -ENOTSUP;
}
...
static int api_dev_config(const struct device *dev,
			  ...
			  const struct mspi_dev_cfg *cfg)
*/
static int api_mspi_xec_dev_config(const struct device *dev, const struct mspi_dev_id *dev_id,
				   const enum mspi_dev_cfg_mask param_mask,
				   const struct mspi_dev_cfg *cfg)
{
	const struct mspi_dw_config *dev_config = dev->config;
...
static int api_mspi_xec_get_channel_status(const struct device *dev, uint8_t ch)
{
	ARG_UNUSED(ch);
...
static int api_transceive(const struct device *dev,
			  ...
			  const struct mspi_xfer *req)
*/
static int api_mspi_xec_transceive(const struct device *dev, const struct mspi_dev_id *dev_id,
				   const struct mspi_xfer *req)
{
	struct mspi_dw_data *dev_data = dev->data;
	int rc;
...
*/
static int api_mspi_xec_register_callback(const struct device *dev,
					  const struct mspi_dev_id *dev_id,
					  const enum mspi_bus_event evt_type,
					  mspi_callback_handler_t cb,
					  struct mspi_callback_context *ctx)
...
*/
static int api_mspi_xec_timing_config(const struct device *dev, const struct mspi_dev_id *dev_id,
				      const uint32_t param_mask, void *cfg)
{
	struct mspi_dw_data *dev_data = dev->data;
...
static int mspi_xec_pm_action(const struct device *dev, enum pm_device_action action)
{
	struct mspi_dw_data *dev_data = dev->data;
...
static int mspi_xec_dev_init(const struct device *dev)
{
	struct mspi_dw_data *dev_data = dev->data;
	const struct mspi_dw_config *dev_config = dev->config;
...
static DEVICE_API(mspi, mspi_xec_drv_api) = {
	.config             = api_mspi_xec_config,
	.dev_config         = api_mspi_xec_dev_config,
	.get_channel_status = api_mspi_xec_get_channel_status,
	.transceive         = api_mspi_xec_transceive,
#if defined(CONFIG_MULTITHREADING)
	.register_callback  = api_mspi_xec_register_callback,
#endif
#if defined(CONFIG_MSPI_TIMING)
...
#define MSPI_XEC_CE_GPIOS(inst)						\
	.ce_gpios = (const struct gpio_dt_spec [])			\
		{ FOREACH_CE_GPIOS_ELEM(inst) },			\
	.ce_gpios_len = DT_INST_PROP_LEN(inst, ce_gpios)

#define MSPI_XEC_DMA_NODE(i, name)    DT_INST_DMAS_CTLR_BY_NAME(i, name)
#define MSPI_XEC_DMA_DEVICE(i, name)  DEVICE_DT_GET(MSPI_XEC_DMA_NODE(i, name))
#define MSPI_XEC_DMA_CHAN(i, name)    DT_INST_DMAS_CELL_BY_NAME(i, name, channel)
#define MSPI_XEC_DMA_TRIGSRC(i, name) DT_INST_DMAS_CELL_BY_NAME(i, name, trigsrc)

#define MSPI_XEC_DMA_INFO(inst)						\
	.tx_dma_dev = MSPI_XEC_DMA_DEVICE(inst, tx),			\
	.rx_dma_dev = MSPI_XEC_DMA_DEVICE(inst, rx),			\
	.tx_dma_chan = MSPI_XEC_DMA_CHAN(inst, tx),			\
	.tx_dma_trigsrc = MSPI_XEC_DMA_TRIGSRC(inst, tx),		\
	.rx_dma_chan = MSPI_XEC_DMA_CHAN(inst, rx),			\
	.rx_dma_trigsrc = MSPI_XEC_DMA_TRIGSRC(inst, rx)

#define MSPI_MCHP_XEC_INST(inst)								\
	PM_DEVICE_DT_INST_DEFINE(inst, mspi_xec_pm_action);					\
	PINCTRL_DT_INST_DEFINE(inst);								\
	VENDOR_SPECIFIC_DATA_DEFINE(inst);							\
	static void mspi_xec_irq_config##inst(void)						\
	{											\
	...
	static struct struct mspi_xec_drvdat mspi_xec_drvdat##inst;				\
	static const struct mspi_xec_drvcfg mspi_xec_drvcfg##inst = {				\
		.qbase = (mm_reg_t)DT_INST_REG_ADDR(inst),					\
		.irq_config = mspi_xec_irq_config##inst,					\
		.clock_frequency = DT_INST_PROP_OR(inst, clock_frequency, MHZ(12)),		\
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(inst),					\
		.tx_fifo_depth = DT_INST_PROP_OR(inst, tx_fifo_depth, 8),			\
		.rx_fifo_depth = DT_INST_PROP_OR(inst, rx_fifo_depth, 8),			\
		IF_ENABLED(DT_INST_NODE_HAS_PROP(inst, ce_gpios), (MSPI_DW_XEC_GPIOS(inst),))	\
		IF_ENABLED(CONFIG_MSPI_DMA, (MSPI_XEC_DMA_INFO(inst),))				\
	...
	DEVICE_DT_INST_DEFINE(inst,					\
		mspi_xec_dev_init, PM_DEVICE_DT_INST_GET(inst),		\
		&dev##inst##_data, &dev##inst##_config,			\
		POST_KERNEL, CONFIG_MSPI_INIT_PRIORITY,			\
		&mspi_xec_drv_api);

DT_INST_FOREACH_STATUS_OKAY(MSPI_MCHP_XEC_INST)
