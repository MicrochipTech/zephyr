                    ...
                    uint8_t old_dyn_addr)
{
    ...
    I3C_DAT_DynamicAddr_write(hwctx, data->DAT_start_addr, target_info->dat_idx, desc->dynamic_addr);
...
static int i3c_mec5_do_CCC(const struct device *dev, struct i3c_ccc_payload *payload)
{
    ...
    if(false == I3C_Is_Current_Role_Master(hwctx)
            && I3C_Is_Current_Role_BusMaster(hwctx)) {
        ret = -EACCES;
        goto exit_ccc;
    }
...
static int i3c_mec5_do_DAA(const struct device *dev)
{
    ...
    for (idx = DAT_first_free_posn; idx < data->DAT_depth; idx++) {
        ...
        if (_drv_i3c_targets_next_DAA_get(data, &target_needs_DAA)) {
            ...
            if (NULL != target_needs_DAA) {
                ...
                I3C_DAT_DynamicAddrAssign_write(hwctx, data->DAT_start_addr, idx, target_needs_DAA->address);
                 /* Mark the free position as used */
    ...
    if (DAA_entries_count) {
        ...
        I3C_DO_DAA(hwctx, DAT_first_free_posn, DAA_entries_count, &pending_xfer_ctxt.node[0].tid);
...
static int i3c_mec5_target_tx_write(const struct device *dev, uint8_t *buf, uint16_t len)
{
    ...
    if (len > xec_data->i3c_cfg_as_tgt.max_write_len) {
        LOG_DBG("[%s] - Target write data len %d greater than SLV MAX WR LEN %d", __FUNCTION__, len, xec_data->i3c_cfg_as_tgt.max_write_len);
...
static bool _drv_i3c_isr_target_xfers(const struct device *dev, uint16_t num_responses)
{
    ...
    for (int i=0; i< num_responses; i++) { /*Note: We are expecting only one response in the ISR */
        ...
        resp_sts = _i3c_tgt_response_sts_get((struct i3c_sec_regs *)regs, &data_len, &tid, &tgt_receive);
        ...
        if (tgt_receive) {
            ...
            if (RESPONSE_TID_DEFTGTS == tid)  { /* Response for DEFSLVS */
                ...
                if (data_len <= data->DAT_depth) {
                    ...
                    I3C_TGT_DEFTGTS_DAT_write(hwctx, data->DCT_start_addr, data->DAT_start_addr, data_len);
                } else {
...
static bool _drv_i3c_isr_target(const struct device *dev, uint32_t intr_sts)
{
    ...
    if(intr_sts & sbit_IBI_UPDATED_STS) {
        ...
        if ((XFER_TYPE_TGT_RAISE_IBI == pending_xfer_ctxt.xfer_type)
            ...
            if (_i3c_tgt_ibi_resp_get((struct i3c_sec_regs *)regs, &ibi_sir_rem_datalen)) {
                pending_xfer_ctxt.xfer_status = 0;
            } else {
    ...
    if(intr_sts & sbit_CCC_UPDATED_STS) {
        ...
        I3C_Target_MRL_MWL_update(hwctx, &data->i3c_cfg_as_tgt.max_read_len, &data->i3c_cfg_as_tgt.max_write_len);
    }
