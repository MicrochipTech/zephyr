...
LOG_MODULE_REGISTER(i3c_mchp, CONFIG_I3C_MCHP_LOG_LEVEL);
			...
			struct i3c_msg *msgs, uint8_t num_msgs)
{
	...
	ret = k_sem_take(&data->sem_xfer, K_MSEC(CONFIG_I3C_MCHP_RW_TIMEOUT_MS));
	if (ret) {
...
static int dw_i3c_target_ibi_raise_hj(const struct device *dev)
{
	...
	ret = k_sem_take(&data->sem_hj, K_MSEC(CONFIG_I3C_MCHP_RW_TIMEOUT_MS));
	if (ret) {
		return ret;
	}
...
static int dw_i3c_init_scl_timing(const struct device *dev, struct i3c_config_controller *ctrl_cfg)
{
	...
	LOG_DBG("I3C Core clock rate: %u Hz", core_rate);

	/* I3C_OD */
	hcnt = DIV_ROUND_UP(core_rate, ctrl_cfg->scl_od_min.high_ns);
	hcnt = CLAMP(hcnt, SCL_I3C_TIMING_CNT_MIN, SCL_I3C_TIMING_CNT_MAX);

	lcnt = DIV_ROUND_UP(core_rate, ctrl_cfg->scl_od_min.low_ns);
	lcnt = CLAMP(lcnt, SCL_I3C_TIMING_CNT_MIN, SCL_I3C_TIMING_CNT_MAX);

	LOG_DBG("OD hcnt: %u lcnt: %u", hcnt, lcnt);
	scl_timing = SCL_I3C_TIMING_HCNT(hcnt) | SCL_I3C_TIMING_LCNT(lcnt);
	sys_write32(scl_timing, config->regs + SCL_I3C_OD_TIMING);

	/* I3C_PP */
	hcnt = DIV_ROUND_UP(core_rate, data->common.ctrl_config.scl.i3c)/2;
	hcnt = CLAMP(hcnt, SCL_I3C_TIMING_CNT_MIN, SCL_I3C_TIMING_CNT_MAX);

	lcnt = DIV_ROUND_UP(core_rate, data->common.ctrl_config.scl.i3c) - hcnt;
	lcnt = CLAMP(lcnt, SCL_I3C_TIMING_CNT_MIN, SCL_I3C_TIMING_CNT_MAX);

	LOG_DBG("PP hcnt: %u lcnt: %u", hcnt, lcnt);
	scl_timing = SCL_I3C_TIMING_HCNT(hcnt) | SCL_I3C_TIMING_LCNT(lcnt);
