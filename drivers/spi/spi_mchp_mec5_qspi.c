			    ...
			    bool async, spi_callback_t cb, void *userdata)
{
	...
	if (data->xfr_flags & MEC5_QSPI_XFR_FLAG_BUSY) {
		ret = -EBUSY;
	}
...
static void mec5_qspi_isr(const struct device *dev)
{
	...
	if ((data->xfr_flags & MEC5_QSPI_XFR_FLAG_START) && (hwsts & MEC_QSPI_IEN_TXB_EMPTY)) {
		/* empty TX FIFO mec_hal_qspi_done returns BUSY. Ignore if starting transfer */
		data->xfr_flags &= (uint32_t)~MEC5_QSPI_XFR_FLAG_START;
	} else if (status != MEC_RET_OK) {
		status = -EIO;
