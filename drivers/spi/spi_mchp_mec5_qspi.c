...
static void set_freq(const struct device *dev, uint32_t freq_hz)
{
	...
	mm_reg_t qb = devcfg->regbase;
	uint32_t fdiv = 0; /* divide by 65536 */
	uint32_t r = 0;
...
static int qspi_uldma_fd2(const struct device *dev, const uint8_t *txb, uint8_t *rxb,
                          size_t xfrlen, uint32_t flags)
{
	...
	sys_set_bits(qb + XEC_QSPI_MODE_OFS, (BIT(XEC_QSPI_MODE_LD_RX_EN_POS) |
	                                      BIT(XEC_QSPI_MODE_LD_TX_EN_POS)));
...
#define XEC_QSPI_ALL_ERRORS (BIT(XEC_QSPI_SR_TXB_ERR_POS) | BIT(XEC_QSPI_SR_RXB_ERR_POS) |\
