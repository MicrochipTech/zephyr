			 ...
			 const struct spi_buf_set *rx_bufs, bool async)
{
	...
	struct mec5_qspi_data *data = dev->data;
	struct spi_context *ctx = &data->ctx;
	uint32_t qflags = 0, xflags = 0, qstatus = 0;
	int ret = 0;
	bool hold_cs = (data->operation & SPI_HOLD_ON_CS) ? true : false;
	...
	if (!hold_cs) {
		qstatus = mec_hal_qspi_hw_status(regs);
		if ((qstatus & MEC_QSPI_STS_ACTIVE) || !(qflags & MEC5_QSPI_ULDMA_FLAG_CLOSE)) {
			mec_hal_qspi_force_stop(regs);
		}
	}
