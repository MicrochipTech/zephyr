 ...
 * Note 3: input pad disable feature is only for ouput only pin functions
 * with open drain buffer type. Input pad disable's purpose is connection of an
 * external pull-up connected to a voltage lower than the pad voltage.
 * For example, external pull-up to 1.8V when the pad is powered by 3.3V.
 * Disabling the input pad prevents sinking of current due to the voltage difference.
 * Side-effects are the input state cannot be read and interrupt detection will not work.
 */
/* #define MCHP_DEBUG_PINCTRL_CONFIG */
#ifdef MCHP_DEBUG_PINCTRL_CONFIG
extern void capture_gpio_control(uint8_t id, uint32_t gpio_ctrl_val);
#endif
...
static int xec_config_pin(uint32_t portpin, uint32_t conf, uint32_t altf)
{
...
#ifdef MCHP_DEBUG_PINCTRL_CONFIG
	if (idx == MCHP_GPIO_0055_ID) {
		capture_gpio_control(0x10, regs->CTRL[idx]); /* DEBUG */
	}
#endif
...
#ifdef MCHP_DEBUG_PINCTRL_CONFIG
	if (idx == MCHP_GPIO_0055_ID) {
		capture_gpio_control(0x11, regs->CTRL[idx]); /* DEBUG */
	}
#endif
	...
	/* NOTE: disabling input pad results in pin input in control and parallel input
	 * register to be always 0. Input pad disable is used for under-voltage protection
	 * and power reduction. This property should only be enabled if the pin function
	 * does not require input.
	 */
	if (conf & BIT(MCHP_XEC_INPUT_PAD_DIS_POS)) {
		pcr1 |= BIT(MCHP_GPIO_CTRL_INPAD_DIS_POS);
	}

	regs->CTRL[idx] = pcr1; /* output state set in control & parallel regs */
#ifdef MCHP_DEBUG_PINCTRL_CONFIG
	if (idx == MCHP_GPIO_0055_ID) {
		capture_gpio_control(0x12, regs->CTRL[idx]); /* DEBUG */
	}
#endif
...
#ifdef MCHP_DEBUG_PINCTRL_CONFIG
	if (idx == MCHP_GPIO_0055_ID) {
		capture_gpio_control(0x13, regs->CTRL[idx]); /* DEBUG */
	}
#endif
	return 0;
}
