...
#define DT_DRV_COMPAT microchip_xec_gpio_ng
...
/* Alternative:
 * Build pin CR1 register value based on flags and compare to current config.
 * If essential config is the same do not touch the pin.
 * Essential config is:
 * b[1:0] = pull up/down select
 * b[3:2] = power gate
 * b[7:4] = interrupt detect. Ignore, interrupt is configured in separate API
 * b[8] = Output-Buf-Type (0=push-pull), 1(open-drain)
 * b[9] = Direction (0=input), (1=output)
 * b[10] = OutCtrlSel (0=CR1 b[16] is writable), (1=ParOut is writable)
 * b[11] = InvertFunc. (0=MuxFunc normal polarity), (1=MuxFunc is inverted)
 * b[14:12] = MUX select alternate function, always program to 0 (GPIO)
 * b[15] = InPad disable
 * b[16] = CR1 controlled output state. When writable the bit is reflected in
 *         pin's parallel output bit in parallel out regs.
 * Always set the following:
 * b[11]=0, b[15]=0
 * If we need to change CR1 then
 * 1. write CR1 with assembled bits and b[10]=0
 * 2. CR1 |= BIT(10) to select output in parallel-out register
 *
 */
...
static int gpio_xec_ng_configure(const struct device *dev, gpio_pin_t pin, gpio_flags_t flags)
{
	const struct gpio_xec_ng_devcfg *devcfg = dev->config;
	...
	if ((flags & GPIO_OUTPUT) != 0) {
		...
		cr1_new &= ~(MEC_GPIO_CR1_DIR_SET(MEC_GPIO_CR1_DIR_OUT));
	}
...
static void gpio_xec_ng_intr_en(uint8_t port, gpio_pin_t pin, enum gpio_int_mode mode)
{
	uint8_t en = 0ui, girq = 0;
	...
	girq = port_girq_tbl[port];

	soc_ecia_girq_ctrl(girq, (uint8_t)pin, en);
}

/* GPIO device is for each bank of 32 GPIO pins.
 * 0 <= pin <= 31
 */
static int gpio_xec_ng_pin_interrupt_config(const struct device *dev, gpio_pin_t pin,
					    enum gpio_int_mode mode, enum gpio_int_trig trig)
{
	const struct gpio_xec_ng_devcfg *devcfg = dev->config;
	uint32_t cr1 = 0, cr1_addr = 0, port_num = 0;
	...
	cr1 &= (uint32_t)~(MEC_GPIO_CR1_INPD_MSK);

	if (MEC_GPIO_CR1_PG_GET(cr1) == MEC_GPIO_CR1_PG_UNPWRD) {
		cr1 &= (uint32_t)~(MEC_GPIO_CR1_PG_MSK);
		cr1 |= MEC_GPIO_CR1_PG_SET(MEC_GPIO_CR1_PG_VTR);
	}
	...
	if (idet_curr == idet) {
		gpio_xec_ng_intr_en(port_num, pin, mode);
		return 0;
	}
	...
	if (idet >= MEC_GPIO_CR1_IDET_RE) {
		/* Issue with HW glitch filter for edge modes
...
static int gpio_xec_ng_port_set_masked_raw(const struct device *dev, uint32_t mask, uint32_t value)
{
	const struct gpio_xec_ng_devcfg *devcfg = dev->config;
...
static int gpio_xec_ng_port_set_bits_raw(const struct device *dev, uint32_t mask)
{
	const struct gpio_xec_ng_devcfg *devcfg = dev->config;
	uint32_t parout_cr_addr = devcfg->parout_addr;
...
static int gpio_xec_ng_port_clear_bits_raw(const struct device *dev, uint32_t mask)
{
	const struct gpio_xec_ng_devcfg *devcfg = dev->config;
	uint32_t parout_cr_addr = devcfg->parout_addr;
...
static int gpio_xec_ng_port_toggle_bits(const struct device *dev, uint32_t mask)
{
	const struct gpio_xec_ng_devcfg *devcfg = dev->config;
	uint32_t parout_cr_addr = devcfg->parout_addr;
	uint32_t regval = sys_read32(parout_cr_addr);
...
static int gpio_xec_ng_port_get_raw(const struct device *dev, uint32_t *value)
{
	const struct gpio_xec_ng_devcfg *devcfg = dev->config;
...
static int gpio_xec_ng_manage_callback(const struct device *dev, struct gpio_callback *callback,
				       bool set)
{
	struct gpio_xec_ng_data *data = dev->data;
...
static int gpio_xec_ng_get_direction(const struct device *port_dev, gpio_port_pins_t map,
				     gpio_port_pins_t *inputs, gpio_port_pins_t *outputs)
{
	const struct gpio_xec_ng_devcfg *devcfg = port_dev->config;
	uint32_t cr1 = 0u, cr1_addr = 0u, inp = 0u, outp = 0u, pin_num = 0;
		...
		if ((map & BIT(pin_pos)) != 0) {
			...
			if (pwr_gate != MEC_GPIO_CR1_PG_UNPWRD) {
				if (dir == MEC_GPIO_CR1_DIR_OUT) {
					outp |= BIT(pin_pos);
				} else if (in_pad_dis == MEC_GPIO_CR1_INPD_EN) {
					inp |= BIT(pin_pos);
				}
			}
...
int gpio_xec_ng_get_config(const struct device *port_dev, gpio_pin_t pin, gpio_flags_t *flags)
{
	const struct gpio_xec_ng_devcfg *devcfg = port_dev->config;
...
static void gpio_xec_ng_port_isr(const struct device *port_dev)
{
	const struct gpio_xec_ng_devcfg *devcfg = port_dev->config;
	struct gpio_xec_ng_data *data = port_dev->data;
...
static DEVICE_API(gpio, gpio_xec_ng_driver_api) = {
	.pin_configure = gpio_xec_ng_configure,
	.port_get_raw = gpio_xec_ng_port_get_raw,
	.port_set_masked_raw = gpio_xec_ng_port_set_masked_raw,
	.port_set_bits_raw = gpio_xec_ng_port_set_bits_raw,
	.port_clear_bits_raw = gpio_xec_ng_port_clear_bits_raw,
	.port_toggle_bits = gpio_xec_ng_port_toggle_bits,
	.pin_interrupt_configure = gpio_xec_ng_pin_interrupt_config,
	.manage_callback = gpio_xec_ng_manage_callback,
#ifdef CONFIG_GPIO_GET_DIRECTION
	.port_get_direction = gpio_xec_ng_get_direction,
#endif
#ifdef CONFIG_GPIO_GET_CONFIG
	.pin_get_config = gpio_xec_ng_get_config,
#endif
};

#define XEC_NG_GPIO_PORT_FLAGS(inst) ((DT_INST_IRQ_HAS_CELL(inst, irq)) ? GPIO_INT_ENABLE : 0)
#define XEC_NG_GPIO_GIRQ(inst)       (MCHP_XEC_ECIA_GIRQ(DT_INST_PROP_BY_IDX(inst, girqs, 0)))

#define XEC_NG_GPIO_PORT(i)                                                                        \
	static int gpio_xec_ng_port_init##i(const struct device *dev)                              \
	{                                                                                          \
		const struct gpio_xec_ng_devcfg *devcfg = dev->config;                             \
		...
		IRQ_CONNECT(DT_INST_IRQN(i), DT_INST_IRQ(i, priority), gpio_xec_ng_port_isr,       \
			    DEVICE_DT_INST_GET(i), 0u);                                            \
		irq_enable(DT_INST_IRQN(i));                                                       \
		return 0;                                                                          \
	}                                                                                          \
	static struct gpio_xec_ng_data gpio_xec_ng_port_data##i;                                   \
	static const struct gpio_xec_ng_devcfg gpio_xec_ng_dcfg##i = {                             \
		.common =                                                                          \
			{                                                                          \
		...
		.flags = XEC_NG_GPIO_PORT_FLAGS(i),                                                \
		.girq = (uint8_t)XEC_NG_GPIO_GIRQ(i),                                              \
	};                                                                                         \
	DEVICE_DT_INST_DEFINE(i, gpio_xec_ng_port_init##i, NULL, &gpio_xec_ng_port_data##i,        \
			      &gpio_xec_ng_dcfg##i, PRE_KERNEL_1, CONFIG_GPIO_INIT_PRIORITY,       \
			      &gpio_xec_ng_driver_api);

DT_INST_FOREACH_STATUS_OKAY(XEC_NG_GPIO_PORT)
