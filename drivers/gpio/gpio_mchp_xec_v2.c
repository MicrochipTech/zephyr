			      ...
			      gpio_pin_t pin, gpio_flags_t flags)
{
	...
	/* if both pulls are enabled the pin is in repeater mode */
...
/*
 * Called by PINCTRL
 * dev = pointer to GPIO port device
 * pin = pin bit position in GPIO port
 * cfg = PINCTRL encoded pin properties:
 *       Pulls, output buffer type, slew rate, drive strength
 * altf = alternate function where 0 = GPIO
 */

static void config_drive_slew(const struct device *dev, int pin, int conf)
{
...
static void config_alt_func(const struct device *dev, int pin, int altf)
{
	uintptr_t pcr1_addr = pin_ctrl_addr(dev, pin);
