 ...
 */
/* #define MCHP_DEBUG_GPIO_CONFIG */
#ifdef MCHP_DEBUG_GPIO_CONFIG
...
#ifdef MCHP_DEBUG_GPIO_CONFIG
	if (pcr1_addr == 0x400810b4u) { /* DEBUG GPIO055 */
		capture_gpio_control(0x30, pcr1);
	}
#endif
...
#ifdef MCHP_DEBUG_GPIO_CONFIG
		if (pcr1_addr == 0x400810b4u) { /* DEBUG GPIO055 */
			capture_gpio_control(0x31, pcr1);
			capture_gpio_control(0x32, sys_read32(pcr1_addr));
		}
#endif
		return 0;
	}
...
#ifdef MCHP_DEBUG_GPIO_CONFIG
		if (pcr1_addr == 0x400810b4u) { /* DEBUG GPIO055 */
			capture_gpio_control(0x33, pcr1);
			capture_gpio_control(0x34, sys_read32(pcr1_addr));
		}
#endif
	}
...
#ifdef MCHP_DEBUG_GPIO_CONFIG
	if (pcr1_addr == 0x400810b4u) { /* DEBUG GPIO055 */
		capture_gpio_control(0x35, pcr1);
		capture_gpio_control(0x36, sys_read32(pcr1_addr));
	}
#endif
...
#ifdef MCHP_DEBUG_GPIO_CONFIG
	if (pcr1_addr == 0x400810b4u) { /* DEBUG GPIO055 */
		capture_gpio_control(0x37, pcr1 | BIT(MCHP_GPIO_CTRL_AOD_POS));
		capture_gpio_control(0x38, sys_read32(pcr1_addr));
	}
#endif
					    ...
					    enum gpio_int_trig trig)
{
	...
	/* HW detects interrupt on input. Return error if input pad disabled */
	if (pcr1 & BIT(MCHP_GPIO_CTRL_INPAD_DIS_POS)) {
		return -ENOTSUP;
	}
...
int gpio_xec_get_config(const struct device *port, gpio_pin_t pin, gpio_flags_t *flags)
{
	...
	if (!(valid_msk & BIT(pin))) {
		return -EINVAL; /* Or should we set *flags = GPIO_DISCONNECTED and return success? */
	}
