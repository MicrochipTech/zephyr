...
#define DT_DRV_COMPAT microchip_xec_cdma
...
#define XEC_CDMA_MAX_CHANS CONFIG_DMA_MCHP_XEC_CDMA_MAX_CHANNELS
...
struct xec_cdma_xdata {
	...
	struct xec_dchan chdata[XEC_CDMA_MAX_CHANS];
};
...
static int xec_cdma_config(const struct device *dev, uint32_t chan, struct dma_config *config)
{
	...
	if ((chan >= XEC_CDMA_MAX_CHANS) || (config == NULL)) {
		return -EINVAL;
	}
...
static int xec_cdma_get_status(const struct device *dev, uint32_t chan, struct dma_status *status)
{
	...
	if ((chan >= XEC_CDMA_MAX_CHANS) || (status == NULL)) {
		return -EINVAL;
	}
...
{
	if ((chan < 0) || (chan >= XEC_CDMA_MAX_CHANS)) {
