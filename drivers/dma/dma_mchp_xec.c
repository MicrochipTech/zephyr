...
#endif
>>>>>>> f97c43d7abd (drivers: dma: microchip: xec: central DMA driver enhancement)
...
static int xec_cdma_stop(const struct device *dev, uint32_t chan)
{
	...
	sys_clear_bits(rb + CDMA_CHAN_CR_OFS,
		       (BIT(CDMA_CHAN_CR_HFC_RUN_POS) | BIT(CDMA_CHAN_CR_SFC_GO_POS) |
			BIT(CDMA_CHAN_CR_ABORT_POS)));
