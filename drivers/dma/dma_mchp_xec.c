...
#define XEC_DMA_CHAN_IES_REG_MSK		0x7u
#define XEC_DMA_CHAN_IES_BERR_POS		0
#define XEC_DMA_CHAN_IES_RAD_ERR_POS		1
...
static int check_blocks(struct dma_block_config *block, uint32_t block_count, uint32_t unit_size)
{
	if (!block) {
		LOG_ERR("XEC DMA bad block pointer");
		return -EINVAL;
	}
 ...
 * source_handshake - 0=HW, 1=SW  TODO Do we need these to be set if Mem2Mem?
			     ...
			     struct dma_config *config)
{
	...
	ret = check_blocks(block, config->block_count, config->source_data_size);
	if (ret) {
		return ret;
	}
			  ...
			  uint32_t src, uint32_t dst, size_t size)
{
	...
	ienable = chregs->ienable;

	chregs->ienable = 0;
	chregs->control = 0;
	chregs->istatus = 0xffu;
	chregs->mem_addr = data->channels[channel].mstart;
	chregs->mem_addr_end = data->channels[channel].mend;
	chregs->dev_addr = data->channels[channel].dstart;
	chregs->ienable = ienable;
	chregs->control = ctrl;
			      ...
			      struct dma_status *status)
{
	...
	/* TODO - Should dma_state.busy reflect the current transfer has
	 * not finished? DMA can be configured to process a linked list
	 * of transfers. If we are not done with the list then set busy true?
	 */
	if (chan_ctrl & BIT(XEC_DMA_CHAN_CTRL_BUSY_POS)) {
		status->busy = true;
		/* number of bytes remaining in channel */
		status->pending_length = chregs->mem_addr_end - chregs->mem_addr;
	} else {
		status->busy = false;
	...
	/* TODO
	 * status->free  free buffer space
	 * status->write_position in current buffer
	 * status->read_position in current buffer
	 * status->total_copied (uint64_t)
	 */
...
#if 0
/* TODO: Driver can be simplified by limiting it to one block at a time.
 * If we choose to do this then implement this API so applications can
 * discover the block count limit.
 */
...
static void dma_xec_irq_handler(const struct device *dev, uint32_t channel)
{
	...
	chan_data->total_count += (regs->mem_addr - chan_data->mstart);
	if (!(sts & BIT(XEC_DMA_CHAN_IES_BERR_POS))) {
		...
		if (chan_data->block_count) {
			if ((chan_data->flags & BIT(DMA_XEC_CHAN_FLAGS_CB_EOB_POS))
			    && chan_data->cb) {
				chan_data->cb(dev, chan_data->user_data, channel, 0);
			}

			chan_data->curr++;
			struct dma_block_config *block = chan_data->curr;

			if (chan_data->dir == PERIPHERAL_TO_MEMORY) {
				regs->mem_addr = block->dest_address;
				regs->mem_addr_end = block->dest_address + block->block_size;
				regs->dev_addr = block->source_address;
			} else {
				regs->mem_addr = block->source_address;
				regs->mem_addr_end = block->source_address + block->block_size;
				regs->dev_addr = block->dest_address;
			}

			if (regs->control & BIT(XEC_DMA_CHAN_CTRL_DIS_HWFL_POS)) {
				regs->control |= BIT(XEC_DMA_CHAN_CTRL_SWFL_GO_POS);
			} else {
				regs->control |= BIT(XEC_DMA_CHAN_CTRL_HWFL_RUN_POS);
			}
		} else { /* done. disable interrupts and invoke callback if present */
			regs->ienable = 0;
			if (chan_data->cb) {
				chan_data->cb(dev, chan_data->user_data, channel, 0);
			}
		}
	} else { /* error, disable interrupts and invoke callback if enabled and present */
		regs->ienable = 0;
		if (!(chan_data->flags & BIT(DMA_XEC_CHAN_FLAGS_CB_ERR_DIS_POS))
		    && chan_data->cb) {
			chan_data->cb(dev, chan_data->user_data, channel, (int)sts);
		}
	}
}
