			     ...
			     struct dma_config *config)
{
	...
	sys_write32(dstart, chregs + XEC_DMA_CHAN_DEV_ADDR_OFS);

	sys_write32(ctrl, chregs + XEC_DMA_CHAN_CTRL_OFS);
	sys_write32(BIT(XEC_DMA_CHAN_IES_BERR_POS) | BIT(XEC_DMA_CHAN_IES_DONE_POS),
		    chregs + XEC_DMA_CHAN_IENABLE_OFS);
	sys_set_bit(chregs + XEC_DMA_CHAN_ACTV_OFS, XEC_DMA_CHAN_ACTV_EN_POS);
			  ...
			  uint32_t src, uint32_t dst, size_t size)
{
	...
	sys_write32(chdata->mstart, chregs + XEC_DMA_CHAN_MEM_ADDR_OFS);
	sys_write32(chdata->mend, chregs + XEC_DMA_CHAN_MEM_ADDR_END_OFS);
	sys_write32(chdata->dstart, chregs + XEC_DMA_CHAN_DEV_ADDR_OFS);
	sys_write32(ctrl, chregs + XEC_DMA_CHAN_CTRL_OFS);
...
static int dma_xec_start(const struct device *dev, uint32_t channel)
{
	...
	sys_write32(0xffu, chregs + XEC_DMA_CHAN_ISTATUS_OFS);
	chan_ctrl = sys_read32(chregs + XEC_DMA_CHAN_CTRL_OFS);
	...
	sys_write32(BIT(XEC_DMA_CHAN_IES_BERR_POS) | BIT(XEC_DMA_CHAN_IES_DONE_POS),
		    chregs + XEC_DMA_CHAN_IENABLE_OFS);
	sys_write32(chan_ctrl, chregs + XEC_DMA_CHAN_CTRL_OFS);
	sys_set_bit(chregs + XEC_DMA_CHAN_ACTV_OFS, XEC_DMA_CHAN_ACTV_EN_POS);
			      ...
			      struct dma_status *status)
{
	...
	if (chan_ctrl & BIT(XEC_DMA_CHAN_CTRL_BUSY_POS)) {
		uint32_t rem = sys_read32(chregs + XEC_DMA_CHAN_MEM_ADDR_END_OFS) -
			       sys_read32(chregs + XEC_DMA_CHAN_MEM_ADDR_OFS);
			      ...
			      enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_RESUME:
		sys_set_bit(regs + XEC_DMA_MAIN_CTRL_OFS, XEC_DMA_MAIN_CTRL_EN_POS);
		break;

	case PM_DEVICE_ACTION_SUSPEND:
		/* sys_clear_bit(regs + XEC_DMA_MAIN_CTRL_OFS, XEC_DMA_MAIN_CTRL_EN_POS); */
		break;
...
static void dma_xec_irq_handler(const struct device *dev, uint32_t channel)
{
	...
	uint32_t sts = sys_read32(regs + XEC_DMA_CHAN_ISTATUS_OFS);
	int cb_status = 0;
	...
	if (idx < 16) {
		...
		channel_isr_ctrl[channel][idx] = sys_read32(regs + XEC_DMA_CHAN_CTRL_OFS);
		channel_isr_idx[channel] = ++idx;
	}
#endif
	LOG_DBG("maddr=0x%08x mend=0x%08x daddr=0x%08x ctrl=0x%08x sts=0x%02x",
		sys_read32(regs + XEC_DMA_CHAN_MEM_ADDR_OFS),
		sys_read32(regs + XEC_DMA_CHAN_MEM_ADDR_END_OFS),
		sys_read32(regs + XEC_DMA_CHAN_DEV_ADDR_OFS),
		sys_read32(regs + XEC_DMA_CHAN_CTRL_OFS), sts);

	sys_write32(0u, regs + XEC_DMA_CHAN_IENABLE_OFS);
	sys_write32(0xffu, regs + XEC_DMA_CHAN_ISTATUS_OFS);
	mchp_xec_ecia_girq_src_clr(info[channel].gid, info[channel].gpos);

	chan_data->isr_hw_status = sts;
	chan_data->total_curr_xfr_len +=
		(sys_read32(regs + XEC_DMA_CHAN_MEM_ADDR_OFS) - chan_data->mstart);
...
static int dma_xec_init(const struct device *dev)
{
	...
	sys_write32(BIT(XEC_DMA_MAIN_CTRL_SRST_POS), regs + XEC_DMA_MAIN_CTRL_OFS);
	sys_write32(0u, regs + XEC_DMA_MAIN_PKT_OFS); /* I/O delay, write to read-only register */
	sys_write32(BIT(XEC_DMA_MAIN_CTRL_EN_POS), regs + XEC_DMA_MAIN_CTRL_OFS);
