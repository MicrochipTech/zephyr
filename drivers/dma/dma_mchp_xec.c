			     ...
			     struct dma_config *config)
{
	...
	if (sys_test_bit(chregs + XEC_DMA_CHAN_CTRL_OFS, XEC_DMA_CHAN_CTRL_BUSY_POS)) {
		return -EBUSY;
	}
...
static void dma_xec_irq_handler(const struct device *dev, uint32_t channel)
{
	...
	sys_write32(sts, regs + XEC_DMA_CHAN_ISTATUS_OFS);
...
static int dma_xec_init(const struct device *dev)
{
	...
	while (sys_test_bit(regs + XEC_DMA_MAIN_CTRL_OFS, XEC_DMA_MAIN_CTRL_SRST_POS) != 0) {
		;
	}
