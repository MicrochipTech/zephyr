...
#define DT_DRV_COMPAT microchip_xec_kscan_v2
...
#define KSCAN_XEC_CONFIG(dev)						\
	((struct kscan_xec_config const *)(dev)->config)

#define KSCAN_XEC_DATA(dev)						\
	((struct kscan_xec_data *const)(dev)->data)

#define KSCAN_XEC_REG_BASE(dev)						\
	((struct kscan_regs *)KSCAN_XEC_CONFIG(dev)->base_addr)

static struct kscan_xec_data kbd_data;

static void drive_keyboard_column(struct kscan_regs *base, int data)
{
...
static uint8_t read_keyboard_row(struct kscan_regs *base)
{
...
static bool read_keyboard_matrix(struct kscan_regs *base, uint8_t *new_state)
{
	uint8_t row;
	uint8_t key_event = 0U;

	for (int col = 0; col < MAX_MATRIX_KEY_COLS; col++) {
		drive_keyboard_column(base, col);

		/* Allow the matrix to stabilize before reading it */
		k_busy_wait(50U);
		row = read_keyboard_row(base);
		new_state[col] = row;
		key_event |= row;
	}

	drive_keyboard_column(base, KEYBOARD_COLUMN_DRIVE_NONE);
...
static void scan_matrix_xec_isr(const void *arg)
{
	...
	const struct kscan_xec_config *const cfg = KSCAN_XEC_CONFIG(dev);
	struct kscan_xec_data *kscan_data = KSCAN_XEC_DATA(dev);

	mchp_xec_ecia_girq_src_clr(cfg->girq, cfg->girq_pos);
	irq_disable(DT_INST_IRQN(0));

	k_sem_give(&kscan_data->poll_lock);
	LOG_DBG(" ");
}
...
{
	struct kscan_xec_data *kscan_data = KSCAN_XEC_DATA(dev);
	struct kscan_regs *base = KSCAN_XEC_REG_BASE(dev);
	uint8_t matrix_new_state[MAX_MATRIX_KEY_COLS] = {0U};
	bool key_pressed = false;
	uint32_t cycles_now  = k_cycle_get_32();

	if (++kscan_data->scan_cycles_idx >= SCAN_OCURRENCES)
		kscan_data->scan_cycles_idx = 0U;

	kscan_data->scan_clk_cycle[kscan_data->scan_cycles_idx] = cycles_now;

	/* Scan the matrix */
	key_pressed = read_keyboard_matrix(base, matrix_new_state);
	...
	for (int c = 0; c < MAX_MATRIX_KEY_COLS; c++) {
		...
		row_changed = matrix_new_state[c] ^
					kscan_data->matrix_previous_state[c];
		...
		for (int r = 0; r < MAX_MATRIX_KEY_ROWS; r++) {
			...
			if (row_changed & BIT(r))
				kscan_data->scan_cycle_idx[c][r] =
					kscan_data->scan_cycles_idx;
		}

		kscan_data->matrix_unstable_state[c] |= row_changed;
		kscan_data->matrix_previous_state[c] = matrix_new_state[c];
	}

	for (int c = 0; c < MAX_MATRIX_KEY_COLS; c++) {
		deb_col = kscan_data->matrix_unstable_state[c];
		...
		for (int r = 0; r < MAX_MATRIX_KEY_ROWS; r++) {
			...
			kscan_data->scan_clk_cycle[kscan_data->scan_cycle_idx[c][r]]);

			/* Does the key requires more time to be debounced? */
			if (debt < (row_bit ? kscan_data->deb_time_press :
						kscan_data->deb_time_rel)) {
				/* Need more time to debounce */
				continue;
			}

			kscan_data->matrix_unstable_state[c] &= ~row_bit;

			/* Check if there was a change in the stable state */
			if ((kscan_data->matrix_stable_state[c] & mask)
								== row_bit) {
			 ...
			 */
			kscan_data->matrix_stable_state[c] ^= mask;
			if (atomic_get(&kscan_data->enable_scan) == 1U) {
				kscan_data->callback(dev, r, c,
					      row_bit ? true : false);
			}
		}
	}
...
void polling_task(void *p1, void *dummy2, void *dummy3)
{
	const struct device *dev = (struct device *)p1;
	const struct kscan_xec_config *const cfg = KSCAN_XEC_CONFIG(dev);
	struct kscan_regs *base = KSCAN_XEC_REG_BASE(dev);
	struct kscan_xec_data *kscan_data = KSCAN_XEC_DATA(dev);
	...
	int64_t local_poll_timeout = kscan_data->poll_timeout;
	...
	while (true) {
		...
		drive_keyboard_column(base, KEYBOARD_COLUMN_DRIVE_ALL);
		base->KSI_STS = MCHP_KSCAN_KSO_SEL_REG_MASK;
		k_sem_take(&kscan_data->poll_lock, K_FOREVER);

		uint32_t start_poll_cycles = k_cycle_get_32();

		while (atomic_get(&kscan_data->enable_scan) == 1U) {
			uint32_t start_period_cycles = k_cycle_get_32();

			if (check_key_events(DEVICE_DT_INST_GET(0))) {
				local_poll_timeout = kscan_data->poll_timeout;
			...
			wait_period =  kscan_data->poll_period -
			 ...
			 */
			if (wait_period > kscan_data->poll_period) {
...
static int kscan_xec_inhibit_interface(const struct device *dev)
{
	...
	atomic_set(&kscan_data->enable_scan, 0);
...
static int kscan_xec_enable_interface(const struct device *dev)
{
	...
	atomic_set(&kscan_data->enable_scan, 1);
...
static struct kscan_xec_config kscan_xec_dev_cfg_0 = {
...
static int kscan_xec_init(const struct device *dev)
{
	...
	kscan_data->deb_time_press = (uint32_t)
		(CONFIG_KSCAN_XEC_DEBOUNCE_DOWN * MSEC_PER_MS);
	kscan_data->deb_time_rel = (uint32_t)
		(CONFIG_KSCAN_XEC_DEBOUNCE_UP * MSEC_PER_MS);
	kscan_data->poll_period = (uint32_t)
		(CONFIG_KSCAN_XEC_POLL_PERIOD * MSEC_PER_MS);
	kscan_data->poll_timeout = 100 * MSEC_PER_MS;

	k_sem_init(&kscan_data->poll_lock, 0, 1);
	atomic_set(&kscan_data->enable_scan, 1);

	k_thread_create(&kscan_data->thread, kscan_data->thread_stack,
			TASK_STACK_SIZE,
			polling_task, (void *)dev, NULL, NULL,
