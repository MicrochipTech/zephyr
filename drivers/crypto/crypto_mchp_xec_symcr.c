...
static int xec_symcr_aes_session_begin(const struct device *dev, struct cipher_ctx *ctx,
...
int xec_symcr_aes_session_free(const struct device *dev, struct cipher_ctx *ctx)
{
	return -ENOTSUP;
}
...
static struct crypto_driver_api xec_symcr_api = {
	...
	.cipher_begin_session = xec_symcr_aes_session_begin,
	.cipher_free_session = xec_symcr_aes_session_free,
