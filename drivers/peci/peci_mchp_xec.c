...
#define PECI_XEC_CONFIG(_dev)				\
	(((const struct peci_xec_config * const)	\
	  _dev->config))

#define PECI_XEC_REG_BASE(_dev)				\
	((struct peci_regs *)				\
	 ((const struct peci_xec_config * const)	\
	  _dev->config)->base)

#define PECI_XEC_DATA(_dev)				\
	((struct peci_xec_data *const)(_dev)->data)

static const struct peci_xec_config peci_xec_0_config = {
...
static struct peci_xec_data peci_xec_0_data;

static int check_bus_idle(struct peci_regs *regs)
{
...
{
	struct peci_regs *regs = PECI_XEC_REG_BASE(dev);
	struct peci_xec_data *data = PECI_XEC_DATA(dev);
	uint16_t value;
...
static int peci_xec_disable(const struct device *dev)
{
	...
	const struct peci_xec_config *cfg = PECI_XEC_CONFIG(dev);
...
#ifdef CONFIG_PECI_INTERRUPT_DRIVEN
	struct peci_xec_config *cfg = PECI_XEC_CONFIG(dev);
...
static void peci_xec_isr(const void *arg)
{
	...
	struct peci_xec_data *data = PECI_XEC_DATA(dev);
	struct peci_regs *regs = PECI_XEC_REG_BASE(dev);
	uint8_t status1 = regs->STATUS1;
	uint8_t status2 = regs->STATUS2;

	/* clear any PECI controller R/W1C status bits */
	if (regs->ERROR) {
		regs->ERROR = regs->ERROR;
	}

	if (status2) {
		regs->STATUS2 = status2;
	}

	if (status1) {
		regs->STATUS1 = status1;
	}

	/* clear latched PECI GIRQ status after clearing PECI controller */
	mchp_soc_ecia_girq_src_clr(cfg->girq, cfg->girq_pos);

	if (status2 & MCHP_PECI_STS2_WFE) {
		LOG_WRN("TX FIFO empty ST2:%x", status2);
		k_sem_give(&data->tx_lock);
	}

	if (status2 & MCHP_PECI_STS2_RFE) {
		LOG_WRN("RX FIFO full ST2:%x", status2);
	}
}
#endif
...
static int peci_xec_init(const struct device *dev)
{
	...
	/* clear R/W1C PECI GIRQ status and NVIC pending status */
	mchp_soc_ecia_girq_src_clr(cfg->girq, cfg->girq_pos);
	NVIC_ClearPendingIRQ(cfg->irq_num);
...
DEVICE_DT_INST_DEFINE(0,
		    &peci_xec_init,
		    NULL,
		    &peci_xec_0_data, &peci_xec_0_config,
