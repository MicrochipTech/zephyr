...
static inline struct xec_mux_root_data *
 ...
 */
static int xec_mux_set_channel(const struct device *root_dev,
				uint8_t port_sel, uint32_t clock_freq)
{
...
{
	const struct xec_mux_root_config *root_cfg =
			get_root_config_from_channel(dev);

	return i2c_configure(root_cfg->parent_dev, dev_config);
}

static int xec_mux_transfer(const struct device *dev, struct i2c_msg *msgs,
			     uint8_t num_msgs, uint16_t addr)
{
	...
	ret = xec_mux_set_channel(ch_cfg->root, ch_cfg->port_sel,
				  ch_cfg->clock_frequency);
	if (ret != 0) {
		goto end_trans;
	}
...
static int xec_mux_channel_init(const struct device *dev)
{
	...
	if (ch_cfg->port_sel >= XEC_I2C_CFG_MAX_PORT) {
		LOG_ERR("Invalid port_sel %u for %s (max %u)",
			ch_cfg->port_sel, dev->name, XEC_I2C_CFG_MAX_PORT - 1);
		return -EINVAL;
	}
...
#define XEC_MUX_CHILD_DEFINE(node_id)						\
	static const struct xec_mux_channel_config				\
		xec_mux_ch_config_##node_id = {					\
	...
	DEVICE_DT_DEFINE(node_id,						\
			 xec_mux_channel_init,					\
			 NULL,							\
			 NULL,							\
			 &xec_mux_ch_config_##node_id,				\
			 POST_KERNEL,						\
			 CONFIG_I2C_XEC_MUX_CHANNEL_INIT_PRIO,			\
			 &xec_mux_api);

#define XEC_MUX_ROOT_DEFINE(inst)						\
	static const struct xec_mux_root_config					\
		xec_mux_root_config_##inst = {					\
		.parent_dev = DEVICE_DT_GET(DT_INST_PHANDLE(inst, i2c_parent)),	\
	};									\
	static struct xec_mux_root_data						\
		xec_mux_root_data_##inst;					\
	DEVICE_DT_INST_DEFINE(inst,						\
			      ...
			      &xec_mux_root_config_##inst,			\
			      POST_KERNEL,					\
			      CONFIG_I2C_XEC_MUX_ROOT_INIT_PRIO,		\
			      NULL);						\
