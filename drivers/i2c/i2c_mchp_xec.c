...
LOG_MODULE_REGISTER(i2c_xec, CONFIG_I2C_LOG_LEVEL);
...
static uint32_t prog_target_addresses(const struct device *dev)
{
	...
	struct i2c_xec_data *const data = dev->data;
	mem_addr_t rb = devcfg->base;
	uint32_t nt = (uint32_t)atomic_get(&data->num_targets_registered);
	uint32_t val = 0, n = 0;

	if (nt > XEC_I2C_MAX_TARGETS) {
		nt = XEC_I2C_MAX_TARGETS;
	}

	for (n = 0; n < nt; n++) {
		struct i2c_target_config *t = data->targets[n];

		val |= XEC_I2C_OA_SET(n, t->address);
	}
			    ...
			    uint8_t num_msgs, uint16_t addr)
{
	...
	if (data->state != XEC_I2C_STATE_OPEN) {
		...
		data->i2c_compl = sys_read32(rb + XEC_I2C_COMP_OFS);
...
static int state_check_ack(struct i2c_xec_data *data)
{
	...
	if (!(data->i2c_sr & BIT(XEC_I2C_SR_LRB_AD0_POS))) { /* ACK? */
...
static void i2c_xec_isr(const struct device *dev)
{
	...
	i2c_xec_isr_compl = sys_read32(rb + XEC_I2C_COMP_OFS);
	...
	sys_write32(XEC_I2C_COMP_RW1C_MSK, rb + XEC_I2C_COMP_OFS);
