...
/*
 * Resets and reconfigures I2C controller.
 * The caller must insure I2C lines are idle (both high) before calling this
 * routine. If I2C SCL and SDA lines are not in idle the controller signal
 * an error after it comes out of reset and samples both lines.
 */
...
/*
 * Spin with timeout for controller to detect the bus is not busy: controller
 * Not Bus Busy (NBB) bit is set.
 * Controller NBB 1->0 : Controller generated START and did not lose arbitration.
 *                       independent of target address ACK/NACK.
 * Controller NBB 0->1 : Controller generated STOP and BUS_IDLE_MIN time has
 *                       expired.
 * Return:
 * 0(OK) NBB -> 1 within timeout and BER is 0
 * -ETIMEDOUT NBB remained 0 for timeout period
 * -EBUSY Bus Error (BER) was detected.
 */
static int wait_bus_free(const struct device *dev)
{
	...
	/* Check for bus error during STOP generation */
 ...
 * returns:
 *  0: I2C.Status.PIN == 0 I2C finished with ACK.
 * -EBUSY Bus Error
 * -ETIMEDOUT I2C.Status.PIN always 1
 * -EIO target device NACK's address or data.
 * NOTES:
 * If I2C.Status.BER == 1 then controller sets I2C.Status PIN and NBB to 1.
 *
 */
static int wait_completion(const struct device *dev)
{
	...
	/*
	 * If we get here then PIN=0 and BER=0. When PIN=0 the LRB_AD0 status
	 * bit contains the last received bit (9th bit). Per I2C protocol
	 * 9th bit == 0 is ACK.
	 */
	if (MCHP_I2C_SMB_STS_RO(ba) & MCHP_I2C_SMB_STS_LRB_AD0) {
		return -EIO; /* Target NACK'd the address or data */
	}
...
poll_wr_done:
	switch (ret) {
	case 0: /* ACK or no error */
		break;
	case -EIO: /* NACK */
		send_stop_and_wait(dev);
		break;
	default: /* Bus Error(-EBUSY), Timeout(-TIMEDOUT), other error */
		recover_from_error(dev);
		break;
	}
...
static void i2c_xec_bus_isr(void *arg)
{
	...
	if (status & MCHP_I2C_SMB_STS_AAS) {
		...
		uint8_t rx_data = MCHP_I2C_SMB_DATA(ba);
