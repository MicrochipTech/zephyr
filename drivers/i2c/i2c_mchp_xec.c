...
enum i2c_xec_isr_state {
	...
	I2C_XEC_ISR_STATE_TM_AAT, /* external Host generated START target_addr match */
	I2C_XEC_ISR_STATE_TM_TX, /* target transmit mode: external Host read */
	I2C_XEC_ISR_STATE_TM_RX, /* target receiver mode: external Host write */
	I2C_XEC_ISR_STATE_TM_STOP, /* detected STOP generated by external Host */
#endif
...
static struct i2c_target_config *find_target(struct i2c_xec_data *data)
{
	struct i2c_target_config *ptc = NULL;
	uint32_t ntargets = (uint32_t)atomic_get(&data->num_targets_registered);

	for (uint32_t n = 0; n < ntargets; n++) {
		if (data->targets[n]->address == data->targ_addr) {
			ptc = data->targets[n];
			break;
		}
	}
...
/* TODO: Spec says AAT is set and FW must poll for PIN==0
 * This imples HW fires interrupt immediately on address match not waiting
 * for 9th clock.
 */
static int state_tm_aat(struct i2c_xec_data *data)
{
	const struct device *dev = data->dev;
	const struct i2c_xec_config *devcfg = dev->config;
	mem_addr_t rb = devcfg->base;
	...
	while ((data->i2c_sr & BIT(XEC_I2C_SR_PIN_POS)) != 0) {
		data->i2c_sr = sys_read8(rb + XEC_I2C_SR_OFS);
	}

	/* read the target address the external Host sent.
	 * Read of data register causes our target HW FSM to stop clock stretching.
	 * The external Host can then continue the transaction.
	 */
	data->targ_addr = sys_read8(rb + XEC_I2C_DATA_OFS);

	if ((data->targ_addr & BIT(0)) != 0) { /* Host read of data from this target? */
		next_state = I2C_XEC_ISR_STATE_TM_TX;
	} else {
		next_state = I2C_XEC_ISR_STATE_TM_RX;
	}

#ifndef CONFIG_I2C_TARGET_BUFFER_MODE
	struct i2c_target_config *ptc = find_target(data);

	if (ptc == NULL) {
		return I2C_XEC_ISR_STATE_MAX; /* this should not happen */
	}

	const struct i2c_target_callbacks *tcbs = ptc->callbacks;
	int rc = 0;

	if (next_state == I2C_XEC_ISR_STATE_TM_RX) { /* Host write to this target? */
		if (tcbs->write_requested != NULL) {
			rc = tcbs->write_requested(ptc);
		}
	} else { /* Host read from this target */
		if (tcbs->read_requested != NULL) {
			rc = tcbs->read_requested(ptc, &data->targ_data);
		}
	}

	if (rc < 0) {
		/* TODO - Host write NAK every byte until STOP
		 * Host read do not supply data or supply dummy data until STOP
		 */
		data->ignore = 1u;
	}
#endif
...
/* Service external Host I2C Read. We transmit data to Host */
static int state_tm_targ_tx(struct i2c_xec_data *data)
{
	const struct device *dev = data->dev;
	const struct i2c_xec_config *devcfg = dev->config;
	mem_addr_t rb = devcfg->base;
	int next_state = I2C_XEC_ISR_STATE_TM_TX;
	uint8_t data_byte = 0xffu;

	if (data->ignore == 0) {
		/* get data from app callback */
	}

	sys_write8(data_byte, rb + XEC_I2C_DATA_OFS);

	/* next state should be check if Host ACK'd */

	return next_state;
}

/* Service external Host I2C Write. We accept data from Host */
static int state_tm_target_rx(struct i2c_xec_data *data)
{
	const struct device *dev = data->dev;
	const struct i2c_xec_config *devcfg = dev->config;
	mem_addr_t rb = devcfg->base;
	int next_state = I2C_XEC_ISR_STATE_TM_RX;


	if (data->ignore == 0) {
		/* invoke write received callback */
	}
...
static void xec_i2c_kwork_thread(struct k_work *work)
{
...
#ifdef CONFIG_I2C_TARGET
	if ((data->i2c_sr & BIT(XEC_I2C_SR_AAT_POS)) != 0) { /* target address match? */
		/* data->xfr_state */
		state = I2C_XEC_ISR_STATE_TM_AAT;
	}
#endif
	...
	while (run_sm) {
		...
		case I2C_XEC_ISR_STATE_EXIT_1:
			...
			break;
#ifdef CONFIG_I2C_TARGET
		case I2C_XEC_ISR_STATE_TM_AAT: /* external Host generated START target_addr match */
			next_state = state_tm_aat(data);
			break;
		case I2C_XEC_ISR_STATE_TM_TX: /* target transmit mode: external Host read */
			next_state = state_tm_targ_tx(data);
			break;
		case I2C_XEC_ISR_STATE_TM_RX: /* target receiver mode: external Host write */
			next_state = state_tm_targ_rx(data);
			break;
		case I2C_XEC_ISR_STATE_TM_STOP: /* detected STOP generated by external Host */
			/* TODO */
			break;
#endif /* CONFIG_I2C_TARGET */
		default:
