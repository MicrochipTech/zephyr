...
static uint32_t get_lines(const struct device *dev)
{
	...
	gpio_port_value_t scl = 0, sda = 0;
	int ret = gpio_port_get_raw(data->scl_gpio, &scl);

	if (ret) {
		return i2c_lines;
	}

	if (scl & BIT(cfg->scl_pos)) {
		i2c_lines |= I2C_LINES_SCL_HI;
	}

	if (data->sda_gpio == data->scl_gpio) { /* same GPIO bank? */
		sda = scl;
	} else {
		ret = gpio_port_get_raw(data->sda_gpio, &sda);
		if (ret) {
			return i2c_lines;
		}
	}

	if (sda & BIT(cfg->sda_pos)) {
...
static int ctrl_tx(const struct device *dev, struct i2c_msg *msg, uint16_t addr)
{
	...
	for (size_t n = 0; n < msg->len; n++) {
		...
		if (data->i2c_status & MCHP_I2C_SMB_STS_LRB_AD0) { /* NACK? */
			do_stop(dev, WAIT_COUNT);
			return -EIO;
		}
	}

	if (mflags & I2C_MSG_STOP) {
		ret = do_stop(dev, WAIT_COUNT);
	}
				   ...
				   struct i2c_slave_config *config)
{
	...
	mchp_soc_ecia_girq_src_clr(cfg->girq_id, cfg->girq_bit);
	mchp_soc_ecia_girq_src_en(cfg->girq_id, cfg->girq_bit);
				     ...
				     struct i2c_slave_config *config)
{
	...
	mchp_soc_ecia_girq_src_dis(cfg->girq_id, cfg->girq_bit);
...
#define I2C_XEC_DEVICE(n)						\
	...
	static const struct i2c_xec_config i2c_xec_config_##n = {	\
		...
		.girq_id = DT_INST_PROP(n, girq_id),			\
