...
#define DT_DRV_COMPAT microchip_xec_i2c_ctrl
...
struct i2c_xec_ctrl_config {
	...
	struct xec_speed_cfg *xec_100k_cfg;
	struct xec_speed_cfg *xec_400k_cfg;
	struct xec_speed_cfg *xec_1000k_cfg;
};

struct i2c_xec_ctrl_data {
	uint8_t state;
...
properties:
  ...
  port-sel:
    ...
    required: true
    description: |
      SoC I2C port pins the controller is connected to at driver
      initialization time. Note pinctrl-0 must contain the pins
      for the selected port. If CONFIG_I2C_PORT_SELECT is enable
      pinctrl-0 should contain pins for all the ports that will
      be used.
  ...
  pcr:
    ...
    required: true
    description:
      Encoded PCR index and bit position used for sleep enable,
      clock required, and block reset. Use values generated by
      the MCHP_XEC_PCR_SCR_ENCODE(idx, bitpos, domain) macro.
      Note, domain is from dt-binding/clock/mchp_xec_pcr.h
      MCHP_XEC_PCR_CLK_PERIPH

  timings-100:
    type: array
    description: |
      I2C 100 KHz controller timing adjustments contains 5 32-bit values:
      Bus clock, data timing, start hold time, timeout scaling and start
      hold time. Please refer to SoC data sheet. If not present driver
      will use default settings.

  timings-400:
    type: array
    description: |
      I2C 400 KHz controller timing adjustments contains 5 32-bit values:
      Bus clock, data timing, start hold time, timeout scaling and start
      hold time. Please refer to SoC data sheet. If not present driver
      will use default settings.
  ...
  timings-1000:
    type: array
    description: |
      I2C 1 MHz controller timing adjustments contains 5 32-bit values:
      Bus clock, data timing, start hold time, timeout scaling and start
      hold time. Please refer to SoC data sheet. If not present driver
      will use default settings.
...
/* Can we use DT_FOREACH_xxx to parse property ctrl-timings
 * and build an array?
 * Every 6 entries (freq_hz, bus_clk, data_timing, start_hold_time, idle_scale, timeout_scale)
 */
#define XEC_I2C_CTRL_T100(n) \
	{
		.bus_clk = 0,
		.data_timing = 0,
		.start_hold_time = 0,
...
#define XEC_I2C_CTRL_100K_CFG(n) \
	COND_CODE_1(DT_INST_NODE_HAS_PROP(n, timings_100), (XEC_I2C_CTRL_T100(n)), (NULL))

#define XEC_I2C_CTRL_400K_CFG(n) \
	COND_CODE_1(DT_INST_NODE_HAS_PROP(n, timings_400), (), (NULL))

#define XEC_I2C_CTRL_1000K_CFG(n) \
	COND_CODE_1(DT_INST_NODE_HAS_PROP(n, timings_1000), (), (NULL))

#define I2C_XEC_CTRL_DEVICE(n)							\
	PINCTRL_DT_INST_DEFINE(n);						\
	static void xec_i2c_ctrl##n##_irq_config_func(void)			\
	{									\
		IRQ_CONNECT(DT_INST_IRQN(n),					\
			    DT_INST_IRQ(n, priority),				\
			    i2c_xec_ctrl_bus_isr,				\
			    DEVICE_DT_INST_GET(n), 0);				\
		irq_enable(DT_INST_IRQN(n));					\
	}									\
										\
	static struct i2c_xec_ctrl_data i2c_xec_ctrl##n##_data;			\
	static const struct i2c_xec_ctrl_config i2c_xec_ctrl##n##_cfg = {	\
		...
		.irq_config_func = xec_i2c_ctrl##n##_irq_config_func,		\
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(n),			\
		.xec_100k_cfg = XEC_I2C_CTRL_100K_CFG(n),			\
		.xec_400k_cfg = NULL, \
		.xec_1000k_cfg = NULL, \
	};								\
	I2C_DEVICE_DT_INST_DEFINE(n, i2c_xec_ctrl_init, NULL,		\
		&i2c_xec_data_##n, &i2c_xec_ctrl_config_##n,		\
		POST_KERNEL, CONFIG_I2C_INIT_PRIORITY,			\
		&i2c_xec_ctrl_driver_api);

DT_INST_FOREACH_STATUS_OKAY(I2C_XEC_CTRL_DEVICE)
