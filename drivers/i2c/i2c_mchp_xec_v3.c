...
static int i2c_xec_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_SUSPEND:
		if (IS_ENABLED(CONFIG_I2C_TARGET) && (data->tg.targ_bitmap != 0)) {
			/* GIRQ13 bits[0:4] -> GIRQ22 bits[1:5] */
			sys_write32(BIT(XEC_I2C_WKSR_SB_POS), rb + XEC_I2C_WKSR_OFS); /* R/W1C */
			sys_set_bit(rb + XEC_I2C_WKCR_OFS, XEC_I2C_WKCR_SBEN_POS);
			soc_ecia_girq_status_clear(XEC_I2C_SMB_WK_GIRQ, devcfg->girq_pos + 1u);
			soc_ecia_girq_ctrl(XEC_I2C_SMB_WK_GIRQ, devcfg->girq_pos + 1u, 1u);
		}
	...
	case PM_DEVICE_ACTION_RESUME:
		if (IS_ENABLED(CONFIG_I2C_TARGET) && (data->tg.targ_bitmap != 0)) {
			...
			sys_write32(BIT(XEC_I2C_WKSR_SB_POS), rb + XEC_I2C_WKSR_OFS);
			soc_ecia_girq_ctrl(XEC_I2C_SMB_WK_GIRQ, devcfg->girq_pos + 1u, 0);
			soc_ecia_girq_status_clear(XEC_I2C_SMB_WK_GIRQ, devcfg->girq_pos + 1u);
		}
		sys_set_bit(rb + XEC_I2C_CFG_OFS, XEC_I2C_CFG_ENAB_POS);
		break;
