...
#define XEC_I2C_TM_SHAD_ADDR_ANOMALY_WAIT_US 20

#define XEC_I2C_GIRQ_DIS 0
#define XEC_I2C_GIRQ_EN  1U

/* I2C controller bit position in wake GIRQ is offset by 1 */
#define XEC_I2C_GIRQ_WK_BIT_OFS 1U

/* I2C idle interrupt timeout in milliseconds */
#define XEC_I2C_IDLE_TIMEOUT_MS 100U

/* I2C transfer timeout in milliseconds */
#define XEC_I2C_XFR_TIMEOUT_MS 100U
...
static int wait_bus_free(const struct device *dev, uint32_t nwait)
{
	...
	/* Bus error: caller must reset controller and if need a bus recovery */
	if ((sts & BIT(XEC_I2C_SR_BER_POS)) != 0) {
		return -EIO;
	}

	/* Lost arbitration: this controller attempted to generate START when
	 * its NBB=0 indicating the bus is busy (SCL/SDA are not idle) then LAB
	 * is set in I2C.STATUS. Return value indicates our HW stopped the transaction
...
static int i2c_xec_reset_config(const struct device *dev, uint32_t freq_hz, uint8_t port)
{
	...
	/* timing registers */
	xec_i2c_prog_standard_timing(dev, data->active_freq);
...
static int i2c_xec_recover_bus(const struct device *dev)
{
	...
	/* controller reporting error after reset, try bus recovery */
...
int i2c_xec_v3_configure(const struct device *dev, uint32_t config, uint8_t port)
{
	struct i2c_xec_v3_data *const data = dev->data;
...
static int i2c_xec_stop(const struct device *dev, uint32_t flags)
{
	...
	if ((sts & BIT(XEC_I2C_SR_NBB_POS)) == 0) {
		...
		if (flags & BIT(0)) { /* detect STOP completion with interrupt */
			...
			rc = k_sem_take(&data->sync_sem, K_MSEC(XEC_I2C_IDLE_TIMEOUT_MS));
		} else {
...
static void i2c_xec_target_arm_wake(const struct device *dev, bool arm_wake)
{
	const struct i2c_xec_v3_config *devcfg = dev->config;
	mem_addr_t rb = devcfg->base;

	if (arm_wake == true) {
		sys_write32(BIT(XEC_I2C_WKSR_SB_POS), rb + XEC_I2C_WKSR_OFS); /* R/W1C */
		sys_set_bit(rb + XEC_I2C_WKCR_OFS, XEC_I2C_WKCR_SBEN_POS);
		soc_ecia_girq_status_clear(XEC_I2C_SMB_WK_GIRQ,
					   devcfg->girq_pos + XEC_I2C_GIRQ_WK_BIT_OFS);
		soc_ecia_girq_ctrl(XEC_I2C_SMB_WK_GIRQ, devcfg->girq_pos + XEC_I2C_GIRQ_WK_BIT_OFS,
				   XEC_I2C_GIRQ_EN);
	} else { /* disarm, resume path */
...
/* If target mode enabled, we must enable wake on externally generated START.
 * For deep sleep this requires enabling GIRQ22 wake clocks feature.
 * Check if targets are registered and controller was designated a wakeup source in
 * device tree.
 */
static bool i2c_xec_is_wakeup_source(const struct device *dev)
{
#ifdef CONFIG_I2C_TARGET
	const struct i2c_xec_v3_config *devcfg = dev->config;
	struct i2c_xec_v3_data *data = dev->data;

	if ((devcfg->wakeup_source != 0) && (data->tg.targ_bitmap != 0)) {
		return true;
	}
#endif
	return false;
...
static int i2c_xec_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	mem_addr_t rb = devcfg->base;
	int rc = 0;
	bool target_wake_enable = i2c_xec_is_wakeup_source(dev);

	LOG_DBG("PM action: %d", (int)action);

	switch (action) {
	case PM_DEVICE_ACTION_SUSPEND:
		if (target_wake_enable == true) {
			i2c_xec_target_arm_wake(dev, true);
		} else {
			rc = pinctrl_apply_state(devcfg->pcfg, PINCTRL_STATE_SLEEP);
			if ((rc != 0) && (rc != -ENOENT)) {
				LOG_ERR("PM suspend pinctrl default error (%d)", rc);
			}
		}
	...
	case PM_DEVICE_ACTION_RESUME:
		if (target_wake_enable == true) {
			i2c_xec_target_arm_wake(dev, false);
		} else {
			rc = pinctrl_apply_state(devcfg->pcfg, PINCTRL_STATE_DEFAULT);
			if ((rc != 0) && (rc != -ENOENT)) {
				LOG_ERR("PM resume pinctrl default error (%d)", rc);
			}
		}
...
#define I2C_XEC_DEVICE(i)                                                                          \
	...
	static const struct i2c_xec_v3_config i2c_xec_v3_cfg_##i = {                               \
		...
		IF_ENABLED(CONFIG_I2C_TARGET,                                                      \
			(.wakeup_source = DT_INST_PROP_OR(i, wakeup_source, 0))) };                \
