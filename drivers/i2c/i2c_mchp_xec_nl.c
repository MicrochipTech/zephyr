...
#define I2C_XEC_NL_EVENT_I2C_IDLE		BIT(4)
...
enum i2c_xec_nl_pm_policy_state_flag {
	I2C_XEC_NL_PM_POLICY_STATE_CM_FLAG,
	I2C_XEC_NL_PM_POLICY_STATE_FLAG_COUNT,
};
#endif
...
struct i2c_xec_nl_data {
	...
	volatile uint8_t ctr_first_start;
...
static int i2c_xec_nl_msg_tx(const struct device *dev, int async)
{
	...
	if ((data->prev_dir == I2C_XEC_DIR_NONE) || (ctx->curr_msg->flags & I2C_MSG_RESTART)) {
		...
		if ((consumed == ctx->msg_len) && (ctx->curr_msg->flags & I2C_MSG_STOP)) {
			...
			data->ctr_stop_req = 1;
			evmsk |= I2C_XEC_NL_EVENT_I2C_IDLE;
		} else {
...
static int do_i2c_stop(const struct device *dev, int async)
{
	...
	if ((cm_cmd & XEC_I2C_CM_RCNT_MSK) == 0) {
		...
		evmsk = I2C_XEC_NL_EVENT_I2C_IDLE | I2C_XEC_NL_EVENT_I2C_CM_ERR;
	} else {
		k_busy_wait(500u);
			       ...
			       uint8_t num_msgs, uint16_t addr)
{
	...
	if (data->state == I2C_XEC_STATE_CLOSED) {
		 ...
		 * If CONFIG_PM_DEVICE_RUNTIME is not defined this becomes a do nothing stub.
		 */
		(void)pm_device_runtime_get(dev);
#elif defined(CONFIG_PM_DEVICE)
		/* i2c_xec_nl_pm_policy_state_lock_get(data, I2C_XEC_NL_PM_POLICY_STATE_CM_FLAG); */
		/*
	...
	while (ctx->msg_count) {
		...
		/* TOOD: if this doesn't break anything remove this check from i2c_xec_nl_msg_tx/rx */
		if (i2c_context_msg_on(ctx)) {
		...
		} else if ((m->len == 0) && (m->flags & I2C_MSG_STOP)) {
	...
	if (ret) { /* reset and reconfigure I2C hardware */
		...
		/* Controller issues STOP after receiving NAK on target address */
		evmsk = I2C_XEC_NL_EVENT_I2C_IDLE;
		ev = k_event_wait(&data->events, evmsk, false, K_MSEC(10));

		do_dma_stop(dev, I2C_MODE_CTRL);
		ctrl_config(dev, 35); /* !!! milliseconds !!! TODO use define or a new DT item */
		data->ctr_stop_req = 0;
	...
	if (data->state == I2C_XEC_STATE_CLOSED) {
#if defined(CONFIG_PM_DEVICE_RUNTIME)
		/* PM API will decrement the usage count and sleep the device if count becomes 0.
		 * If CONFIG_PM_DEVICE_RUNTIME is not defined this becomes a do nothing stub.
		 */
		(void)pm_device_runtime_put(dev);
#elif defined(CONFIG_PM_DEVICE)
		/* i2c_xec_nl_pm_policy_state_lock_put(data, I2C_XEC_NL_PM_POLICY_STATE_CM_FLAG); */
		/* (void)pm_device_runtime_put(dev); */
		pm_device_busy_clear(dev);
#endif
	}
...
static void i2c_xec_nl_turn_around(const struct device *dev)
{
	const struct i2c_xec_nl_config * const cfg = dev->config;
	struct i2c_xec_nl_data * const data = dev->data;
	struct i2c_context *ctx = &data->ctx;
	...
	regs->config |= (BIT(XEC_I2C_CFG_IDLE_IEN_POS) | BIT(XEC_I2C_CFG_CMD_IEN_POS));
	regs->cm_cmd |= (BIT(XEC_I2C_CM_MPROCEED_POS) | BIT(XEC_I2C_CM_MRUN_POS));
	dma_start(cfg->dma_dev, cfg->ctr_dma_chan);
}

#ifdef CONFIG_I2C_CALLBACK
static void i2c_xec_nl_done(const struct device *dev)
{
	struct i2c_xec_nl_data * const data = dev->data;
...
static void i2c_xec_nl_async_handler(const struct device *dev)
{
	const struct i2c_xec_nl_config * const cfg = dev->config;
	struct i2c_xec_nl_data * const data = dev->data;
	struct i2c_context *ctx = &data->ctx;
	...
	/* TODO - What about the time from error detection to STOP?
	 * If application calls driver before STOP is done how can
	 * we prevent a potentiol problem?
	 */
	if (data->i2c_compl & XEC_I2C_COMPL_CM_ERR) {
		regs->config |= BIT(XEC_I2C_CFG_IDLE_IEN_POS);
		i2c_xec_nl_done(dev);
		if (data->cb) {
			data->cb(dev, -EIO, data->user_data);
		}
	...
	/* TODO - Not an error.
	 * IDLE - due to an I2C STOP in the middle of a sequence of messages. Go to next message
	 *      - due to I2C STOP after an error, do done context cleanup, disable interrupts, and exit
	 * Turn-around: TX to RX transition where CM_DONE status set and MPROC=0 and MRUN=1
	 *   call rx subroutine to run HW or must be manually configure RX HW?
	 * Data completion: write or read count reached 0. HW sets CM_DONE.
	 *   If more data, call tx or rx subroutines to run HW
	 */

	/* Did HW FSM clear MPROCEED for DMA direction change from TX to RX */
	if ((regs->cm_cmd & (BIT(XEC_I2C_CM_MRUN_POS) | BIT(XEC_I2C_CM_MPROCEED_POS)))
	    == BIT(XEC_I2C_CM_MRUN_POS)) {
		i2c_xec_nl_turn_around(dev);
		return;
	}

	i2c_context_update(ctx, data->consumed);
	data->consumed = 0;
	data->prev_dir = data->curr_dir;
	if (ctx->msg_count && i2c_context_msg_on(ctx)) {
		struct i2c_msg *m = ctx->curr_msg;

		if (m->flags & I2C_MSG_READ) {
			data->curr_dir = I2C_XEC_DIR_RD;
			i2c_xec_nl_msg_rx(dev, 1);
		} else {
			data->curr_dir = I2C_XEC_DIR_WR;
			i2c_xec_nl_msg_tx(dev, 1);
		}
	} else {
		i2c_xec_nl_done(dev);
		if (data->cb) {
			data->cb(dev, 0, data->user_data);
		}
	}
}
#endif /* CONFIG_I2C_CALLBACK */
 ...
 */
static void i2c_xec_nl_isr(void *arg)
{
	...
	i2c_sts = regs->ctr_sts;
	compl = regs->compl;
	i2c_config = regs->config;

	if (!(compl & BIT(XEC_I2C_COMPL_IDLE_STS_POS))) {
		regs->config = i2c_config | BIT(XEC_I2C_CFG_IDLE_IEN_POS);
	} else {
		regs->config = i2c_config & ~BIT(XEC_I2C_CFG_IDLE_IEN_POS);
	}

	cm_cmd = regs->cm_cmd;
	data->i2c_status = i2c_sts;
	...
	if (compl & BIT(XEC_I2C_COMPL_IDLE_STS_POS)) {
		data->ctr_stop_req = 0;
		events |= I2C_XEC_NL_EVENT_I2C_IDLE;
				...
				enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_RESUME:
		if (cfg->wake_source) {
			regs->wake_en = 0;
			regs->wake_sts = BIT(0);
			mchp_xec_ecia_info_girq_src_dis(cfg->wake_pll_ecia_info);
			mchp_xec_ecia_info_girq_src_clr(cfg->wake_pll_ecia_info);
		} else {
			if (data->i2c_config & BIT(XEC_I2C_CFG_EN_POS)) {
				regs->config |= BIT(XEC_I2C_CFG_EN_POS);
			}
			ret = pinctrl_apply_state(cfg->pcfg, PINCTRL_STATE_DEFAULT);
			if (ret < 0) {
				LOG_ERR("PINCTRL resume: default state error %d", ret);
			}
		}
		break;
	...
	case PM_DEVICE_ACTION_SUSPEND:
		...
		if (cfg->wake_source) {
			regs->wake_sts = BIT(0);
			regs->wake_en = BIT(0);
			/* configure GIRQ22 PLL wake for this controller */
			mchp_xec_ecia_info_girq_src_clr(cfg->wake_pll_ecia_info);
			mchp_xec_ecia_info_girq_src_en(cfg->wake_pll_ecia_info);
		} else {
			regs->config &= ~BIT(XEC_I2C_CFG_EN_POS);
			...
			if ((ret == 0) || (ret == -ENOENT)) {
				...
				LOG_ERR("PINCTRL suspend apply state error %d", ret);
			}
		}
		break;
...
static int i2c_xec_nl_init(const struct device *dev)
{
	...
	/* pm_device_init_suspended(dev); we initialized as active */
	ret = pm_device_runtime_enable(dev); /* this will suspend the device if state is active */
	if (ret) {
		LOG_ERR("XEC I2C-NL error PM RT enable: %d", ret);
	}
#endif
	return 0;
}
...
#define I2C_NL_XEC_DEVICE(n)						\
	...
	static const struct i2c_xec_nl_config i2c_xec_nl_config_##n = {	\
		...
		.wake_source = DT_INST_PROP_OR(n, wakeup_source, 0),	\
		.wake_pll_ecia_info = DT_INST_PROP_OR(n, wake_pll_ecia_info, 0), \
	};								\
	PM_DEVICE_DT_INST_DEFINE(n, i2c_xec_nl_pm_action);		\
	I2C_DEVICE_DT_INST_DEFINE(n, i2c_xec_nl_init,			\
