				    ...
				    uint8_t num_msgs, uint16_t addr)
{
	...
	ret = mchp_xec_i2c_ctrl_transfer(cfg->ctrl, cfg->port_idx, cfg->bitrate, msgs,
					 num_msgs, addr);
	mchp_xec_i2c_ctrl_mutex_unlock(cfg->ctrl);
	return ret;
}
				       ...
				       void *userdata)
{
	...
	ret = mchp_xec_i2c_ctrl_transfer_cb(cfg->ctrl, cfg->port_idx, cfg->bitrate, msgs,
					    num_msgs, addr, cb, userdata);
	if (ret != 0) {
		mchp_xec_i2c_ctrl_mutex_unlock(cfg->ctrl);
	}
	return ret;
...
static int i2c_xec_v3_port_target_register(const struct device *dev,
...
#define I2C_XEC_V3_PORT_INIT(n)                                                                    \
	...
	I2C_DEVICE_DT_INST_DEFINE(n, i2c_xec_v3_port_init, NULL, NULL,                             \
				  &i2c_xec_v3_port_cfg_##n, PRE_KERNEL_1,                          \
