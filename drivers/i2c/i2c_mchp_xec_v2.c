...
#define I2C_XEC_TARG_GEN_CALL_IDX 2
#define I2C_XEC_TARG_SMB_HA_IDX   3
#define I2C_XEC_TARG_SMB_DA_IDX   4
...
static void prog_target_addresses(const struct device *dev)
{
	...
	for (n = 0; n < I2C_XEC_TARG_ADDR_MAX; n++) {
...
static int i2c_xec_bb_recover(const struct device *dev)
{
	...
	for (int i = 0; i < I2C_XEC_RECOVER_SDA_LOW_RETRIES; i++) {
		...
		for (int j = 0; j < 9; j++) {
			/* drive SCL low */
			/* SCL output drive low, SDA tri-state input */
...
static int i2c_xec_cfg(const struct device *dev, uint32_t dev_config_raw)
{
	...
	data->port_sel = I2C_XEC_PORT_GET(dev_config_raw);
#else
...
static void adjust_num_registered_targets(struct i2c_xec_target *ptg, uint8_t increment)
{
	if (increment != 0) {
		if (ptg->ntargets < I2C_XEC_TARG_ADDR_MAX) {
			ptg->ntargets++;
		}
	} else {
		if (ptg->ntargets != 0) {
			       ...
			       uint8_t enable)
{
	...
	if (enable != 0) {
		...
		sys_clear_bit(rb + XEC_I2C_CFG_OFS, XEC_I2C_CFG_GC_DIS_POS);
		adjust_num_registered_targets(ptg, 1u);
	} else {
		sys_set_bit(rb + XEC_I2C_CFG_OFS, XEC_I2C_CFG_GC_DIS_POS);
		ptg->tcfgs[I2C_XEC_TARG_GEN_CALL_IDX] = NULL;
		adjust_num_registered_targets(ptg, 0);
	}
	...
	if ((data->i2c_sr & BIT(XEC_I2C_SR_AAT_POS)) != 0) {
		...
		if ((ptg->targ_addr & BIT(0)) != 0) { /* Host requesting read from target */
			if ((tcbs != NULL) && (tcbs->read_requested != NULL)) {
				rc = tcbs->read_requested(ptg->curr_target, &ptg->targ_data);
				if (rc == 0) {
					ptg->targ_ignore = 0;
				}
			}
			...
			if ((tcbs != NULL) && (tcbs->write_requested != NULL)) {
				rc = tcbs->write_requested(ptg->curr_target);
				if (rc == 0) {
					ptg->targ_ignore = 0u;
				}
			}
