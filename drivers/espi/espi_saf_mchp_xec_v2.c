...
/* !!!!!!!!!!!!! TODO RPMC Commands !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 * MCHP_SAF_ECP_CMD_RPMC_OP1_CS0	0x03u
 * MCHP_SAF_ECP_CMD_RPMC_OP2_CS0	0x04u
 * MCHP_SAF_ECP_CMD_RPMC_OP1_CS1	0x83u
 * MCHP_SAF_ECP_CMD_RPMC_OP2_CS1	0x84u
 */

/*
 * EC access to SAF atttached flash array
 * Allowed commands:
 * MCHP_SAF_ECP_CMD_READ	0x00u
 * MCHP_SAF_ECP_CMD_WRITE	0x01u
 * MCHP_SAF_ECP_CMD_ERASE	0x02u
 */
			  ...
			  struct espi_saf_packet *pckt, uint8_t cmd)
{
	...
	if (regs->SAF_ECP_BUSY & (MCHP_SAF_ECP_EC0_BUSY |
				  MCHP_SAF_ECP_EC1_BUSY)) {
		LOG_ERR("SAF EC Portal is busy");
		return -EBUSY;
	}

	if ((cmd == MCHP_SAF_ECP_CMD_READ) ||
	    (cmd == MCHP_SAF_ECP_CMD_WRITE)) {
...
/* !!!! TODO !!!!
 * Need updates to zephyr/include/drivers/espi.h
 * enum espi_channel add SAF ???
 * enum espi_bus_event - Add SAF notification ???
 */
