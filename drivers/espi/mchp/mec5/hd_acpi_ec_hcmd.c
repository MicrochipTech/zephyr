			...
			uint32_t *udata, uint32_t flags)
{
	...
	case ECUSTOM_HOST_CMD_GET_PARAM_MEMORY_SIZE:
		*udata = espi_mec_shm_size_get(devcfg->parent, ECUSTOM_HOST_CMD_GET_PARAM_MEMORY_SIZE);
		break;
	default:
		ret = -ENOTSUP;
	}
...
struct mec5_espi_lpc_req {
	...
	int (*rd_req)(const struct device *, enum lpc_peripheral_opcode, uint32_t *, uint32_t);
	int (*wr_req)(const struct device *, enum lpc_peripheral_opcode, uint32_t *, uint32_t);
};
				     ...
				     uint32_t *data, uint32_t flags)
{
	...
	for (unsigned i = 0; i < ARRAY_SIZE(aec_hcmd_lpc_req_tbl); i++) {
