...
struct espi_ct_sig_handler {
	void (*vw_handler)(const struct device *dev);
};

const struct espi_ct_sig_handler espi_ct_vw_sh_tbl[] = {
...
#define GIRQ24_BASE_ADDR ((ECIA_BASE_ADDR) + (20u * (24u - 8u)))
#define GIRQ25_BASE_ADDR ((ECIA_BASE_ADDR) + (20u * (25u - 8u)))

/* Aggregated GIRQ24 */
static void espi_vw_ct_0_6_isr(const struct device *dev)
{
	struct girq_regs *girq24 = (struct girq_regs *)GIRQ24_BASE_ADDR;
	uint32_t result = girq24->RESULT;

	girq24->SRC = result; /* clear all as quickly as possible */
	for (uint8_t n = 0; n < 28u; n++) {
		if (!result) {
			break;
		}
		if (result & BIT(n)) {
			if (espi_ct_vw_sh_tbl[n].vw_handler) {
				espi_ct_vw_sh_tbl[n].vw_handler(dev);
			}
			result &= (uint32_t)~BIT(n);
		}
	}
}

/* Aggregated GIRQ25 */
static void espi_vw_ct_7_10_isr(const struct device *dev)
{
	struct girq_regs *girq25 = (struct girq_regs *)GIRQ25_BASE_ADDR;
	uint32_t result = girq25->RESULT;

	girq25->SRC = result; /* clear all as quickly as possible */
	for (uint8_t n = 0; n < 16; n++) {
		...
		if (result & BIT(n)) {
			...
			if (espi_ct_vw_sh_tbl[n].vw_handler) {
				espi_ct_vw_sh_tbl[idx].vw_handler(dev);
			}
			result &= (uint32_t)~BIT(n);
		}
	}
...
static void espi_xec_connect_irqs(const struct device *dev)
{
	...
	/* Virtual Wires use aggregated GIRQ24 and GIRQ25
	 * GIRQ24 handles Controller-to-Target VW groups 0 - 6
	 * GIRQ25 handles Controller-to-Target VW groups 7 - 10
	 */
	IRQ_CONNECT(DT_INST_IRQ_BY_NAME(0, vw_ct_0_6, irq),
		    DT_INST_IRQ_BY_NAME(0, vw_ct_0_6, priority), espi_vw_ct_0_6_isr,
		    DEVICE_DT_INST_GET(0), 0);
	irq_enable(DT_INST_IRQ_BY_NAME(0, vw_ct_0_6, irq));
	mchp_xec_ecia_girq_aggr_en(24u, 1u);

	IRQ_CONNECT(DT_INST_IRQ_BY_NAME(0, vw_ct_7_10, irq),
		    DT_INST_IRQ_BY_NAME(0, vw_ct_7_10, priority), espi_vw_ct_7_10_isr,
		    DEVICE_DT_INST_GET(0), 0);
	irq_enable(DT_INST_IRQ_BY_NAME(0, vw_ct_7_10, irq));
	mchp_xec_ecia_girq_aggr_en(25u, 1u);
...
{
	for (size_t n = 0; n < ARRAY_SIZE(espi_ct_vw_sh_tbl); n++) {
		if (espi_ct_vw_sh_tbl[n].vw_handler == NULL) {
			continue;
		}

		uint8_t msvw_idx = n / 4u;
		uint8_t msvw_src_id = n % 4u;
		uint8_t girq_id = (n < 28u) ? 24u : 25u;
		uint8_t girq_pos = (n < 28u) ? (uint8_t)(n & 0x1fu) : ((n - 28u) & 0x1fu);

		xec_espi_vw_intr_ctrl(dev, msvw_idx, msvw_src_id, MSVW_IRQ_SEL_EDGE_BOTH);
		mchp_xec_ecia_girq_src_en(girq_id, girq_pos);
	}
