...
static void configure_debug_interface(void)
{
	 ...
	 */
	ecs_hw->DEBUG_CTRL = (MCHP_ECS_DCTRL_DBG_EN |
		...
		ecs_hw->ETM_CTRL = MCHP_ECS_ETM_CTRL_EN;
		ecs_hw->DEBUG_CTRL = (MCHP_ECS_DCTRL_DBG_EN |
				      MCHP_ECS_DCTRL_MODE_SWD);
...
static int board_pinmux_init(const struct device *dev)
{
	 ...
	 */
#ifdef CONFIG_SOC_MEC172X_VTR3_1_8V
...
#ifdef CONFIG_SOC_MEC172X_VCI_PINS_AS_GPIOS
...
#endif
	/* GPIO 0101 and 0102 default to BGPO 0 and 1 */
	pinmux_pin_set(portd, MCHP_GPIO_101, MCHP_GPIO_CTRL_MUX_F0);
	pinmux_pin_set(portd, MCHP_GPIO_102, MCHP_GPIO_CTRL_MUX_F0);
...
#ifndef CONFIG_SOC_MEC172X_DEBUG_AND_ETM_TRACING
	...
	i2c_pinmux(&pinmux_ports, DT_PROP(DT_NODELABEL(i2c_smb_0), port_sel));
#endif

#if DT_NODE_HAS_STATUS(DT_NODELABEL(i2c_smb_1), okay)
	i2c_pinmux(&pinmux_ports, DT_PROP(DT_NODELABEL(i2c_smb_1), port_sel));
#endif

#if DT_NODE_HAS_STATUS(DT_NODELABEL(i2c_smb_2), okay)
	i2c_pinmux(&pinmux_ports, DT_PROP(DT_NODELABEL(i2c_smb_2), port_sel));
#endif

#if DT_NODE_HAS_STATUS(DT_NODELABEL(i2c_smb_3), okay)
	i2c_pinmux(&pinmux_ports, DT_PROP(DT_NODELABEL(i2c_smb_3), port_sel));
#endif

#if DT_NODE_HAS_STATUS(DT_NODELABEL(i2c_smb_4), okay)
	i2c_pinmux(&pinmux_ports, DT_PROP(DT_NODELABEL(i2c_smb_3), port_sel));
#endif
	...
	struct ecs_regs *ecs_hw =
		(struct ecs_regs *)DT_REG_ADDR(DT_NODELABEL(ecs0));

	ecs_hw->PECI_DIS = 0x00u;
	mchp_soc_pcr_periph_slp_ctrl(PCR_PECI, MCHP_PCR_SLEEP_DIS);
