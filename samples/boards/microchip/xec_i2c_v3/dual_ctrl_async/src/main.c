...
static int test_pca9555(struct ctrl_thread_ctx *c)
{
	...
	rc = i2c_write_read_cb_dt(&c->dev, c->msgs, 2U, (const void *)c->tx, 1U,
				  (void *)c->rx, 2U, app_i2c_cb_func, (void *)&c->cb);
	if (rc != 0) {
...
static int test_fram(struct ctrl_thread_ctx *c)
{
	...
	rc = i2c_transfer_cb(c->dev.bus, c->msgs, 1U, c->dev.addr, app_i2c_cb_func,
			     (void *)&c->cb);
	if (rc != 0) {
	...
	rc = i2c_write_read_cb_dt(&c->dev, c->msgs, 2U, (const void *)c->tx, 2U,
				  (void *)c->rx, nbytes, app_i2c_cb_func, (void *)&c->cb);
	if (rc != 0) {
		LOG_ERR("%s: write_read_cb submit error (%d)", c->name, rc);
		return rc;
	}
...
static void worker(struct ctrl_thread_ctx *c, int (*test_fn)(struct ctrl_thread_ctx *))
{
	uint64_t logged = 0;
...
int main(void)
{
	...
	k_thread_create(&thread_a, stack_a, K_THREAD_STACK_SIZEOF(stack_a), thread_a_entry,
			&ctx_a, NULL, NULL, THREAD_PRIORITY, 0, K_NO_WAIT);
	k_thread_name_set(&thread_a, "i2c_a");

	k_thread_create(&thread_b, stack_b, K_THREAD_STACK_SIZEOF(stack_b), thread_b_entry,
			&ctx_b, NULL, NULL, THREAD_PRIORITY, 0, K_NO_WAIT);
