...
#define ESPI_HOST_THREAD_STACK_SIZE 1024
#define ESPI_HOST_THREAD_PRIORITY   7
...
static uint8_t c2t_vw_host_idxs[] = {0x02u, 0x03u, 0x07u, 0x41u, 0x42u, 0x43u, 0x44u, 0x47u, 0x4au};
...
static void espi_host_emu_thread(void *p1, void *p2, void *p3);

K_THREAD_DEFINE(espi_host_tid, ESPI_HOST_THREAD_STACK_SIZE, espi_host_emu_thread, NULL, NULL, NULL,
		ESPI_HOST_THREAD_PRIORITY, 0, 0);

int main(void)
{
	int ret = 0;

	LOG_INF("MEC5 eSPI Host and Target test on same board: %s", DT_N_COMPAT_MODEL_IDX_0);

	ret = espi_target_init(0);
	if (ret) {
		LOG_ERR("eSPI target init error (%d)", ret);
...
static void espi_host_emu_thread(void *p1, void *p2, void *p3)
{
	...
	LOG_INF("eSPI Host Emu Thread entry");
	...
	ret = espi_hc_ctx_emu_put_mult_host_index(
		&hc, c2t_vw_host_idxs, ARRAY_SIZE(c2t_vw_host_idxs), ESPI_HC_EMU_PUT_VW_RST_VAL);
	if (ret) {
	...
	k_sleep(K_MSEC(1000));
	/* Host set Target's VCC_PWRGD input Low */
	/* espi_hc_emu_vcc_pwrgd(0); */

	LOG_INF("eSPI Host EMU enter test loop");

	while (1) {
		k_sleep(K_MSEC(1000));

		LOG_INF("eSPI Host: Send VW PLTRST# = 0");
		espi_hc_set_ct_vwire(&hc, 0x03u, 1, 0);
		espi_hc_ctx_emu_put_host_index(&hc, 0x03u);
		...
		k_sleep(K_MSEC(1000));
		LOG_INF("eSPI Host: Send VW PLTRST# = 1");
		espi_hc_set_ct_vwire(&hc, 0x03u, 1, 1);
		espi_hc_ctx_emu_put_host_index(&hc, 0x03u);
