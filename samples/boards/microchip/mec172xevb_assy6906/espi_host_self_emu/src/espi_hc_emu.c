...
const struct gpio_dt_spec espi_reset_n_dt =
	GPIO_DT_SPEC_GET_BY_IDX(ZEPHYR_USER_NODE, host_emu_gpios, 0);
...
const struct gpio_dt_spec espi_io1_dt =
	GPIO_DT_SPEC_GET_BY_IDX(ZEPHYR_USER_NODE, host_emu_gpios, 1);

const struct gpio_dt_spec trig_gpio_dt =
	GPIO_DT_SPEC_GET_BY_IDX(ZEPHYR_USER_NODE, host_emu_gpios, 2);

const struct gpio_dt_spec vcc_pwrgd_gpio_dt =
	GPIO_DT_SPEC_GET_BY_IDX(ZEPHYR_USER_NODE, host_emu_gpios, 3);

const struct gpio_dt_spec target_ready_n_dt =
	GPIO_DT_SPEC_GET_BY_IDX(ZEPHYR_USER_NODE, host_emu_gpios, 4);
...
static int qspi_init(struct qmspi_regs *regs, uint32_t freq_hz, uint8_t cs)
{
	uint32_t fdiv;
...
{
	struct qmspi_regs *qr = (struct qmspi_regs *)DT_REG_ADDR(DT_NODELABEL(spi0));
	int ret;
...
static void pr_qspi_regs(struct qmspi_regs *qr)
{
...
{
	if (status & (MCHP_QMSPI_STS_TXB_ERR | MCHP_QMSPI_STS_RXB_ERR | MCHP_QMSPI_STS_PROG_ERR |
		      MCHP_QMSPI_STS_LDMA_RX_ERR | MCHP_QMSPI_STS_LDMA_TX_ERR)) {
		return -EIO;
	}
...
int espi_hc_close(void)
{
	...
	status = qr->STS;
	if (status & MCHP_QMSPI_STS_ACTIVE_RO) {
		/* de-assert QSPI chip select */
		qr->EXE = MCHP_QMSPI_EXE_STOP;
		status = qr->STS;
		while (status & MCHP_QMSPI_STS_ACTIVE_RO) {
			status = qr->STS;
		}
		return 0;
	}
...
int espi_hc_fd_tx_fifo(const uint8_t *msg, size_t msglen, uint32_t flags)
{
	...
	if (flags & BIT(0)) {
		...
		qr->LDRX[0].CTRL = 0;
		qr->LDTX[0].CTRL = 0;
		qr->MODE &= ~(MCHP_QMSPI_M_LDMA_RX_EN | MCHP_QMSPI_M_LDMA_TX_EN);
		qr->LDMA_RX_DESCR_BM = 0;
		qr->LDMA_TX_DESCR_BM = 0;
		qr->EXE = MCHP_QMSPI_EXE_CLR_FIFOS;
	...
	qr->STS = UINT32_MAX;
	ctrl = (MCHP_QMSPI_C_IFM_1X | MCHP_QMSPI_C_TX_DATA | MCHP_QMSPI_C_XFR_UNITS_1);
	ctrl |= ((msglen & 0x7fffu) << MCHP_QMSPI_C_XFR_NUNITS_POS);
	if (flags & BIT(1)) {
		ctrl |= MCHP_QMSPI_C_CLOSE;
	}
	...
	while (ntx) {
		status = qr->STS;
		if (status & MCHP_QMSPI_STS_TXBF_RO) {
			break;
		}
	 ...
	 */
	status = qr->STS;
	while (!(status & MCHP_QMSPI_STS_TXBE_RO)) {
		status = qr->STS;
	}
	 ...
	 */
	status = qr->STS;
	while (!(status & MCHP_QMSPI_STS_DONE)) {
		status = qr->STS;
	}
...
int espi_hc_fd_tar(void)
{
	...
	qr->STS = UINT32_MAX;
	ctrl = (MCHP_QMSPI_C_IFM_1X | MCHP_QMSPI_C_TX_DIS | MCHP_QMSPI_C_XFR_UNITS_BITS |
...
int espi_hc_fd_rx_fifo(uint8_t *msg, size_t msglen, uint32_t flags)
{
	...
	qr->STS = UINT32_MAX;
	ctrl = (MCHP_QMSPI_C_IFM_1X | MCHP_QMSPI_C_TX_DIS | MCHP_QMSPI_C_RX_EN |
		MCHP_QMSPI_C_XFR_UNITS_1);
	ctrl |= MCHP_QMSPI_C_XFR_NUNITS(msglen & 0x7fffu);
	if (flags & BIT(1)) {
		ctrl |= MCHP_QMSPI_C_CLOSE;
	}
	qr->CTRL = ctrl; /* write all bits at once */
	...
	while (nrx) {
		status = qr->STS;
		if (status & MCHP_QMSPI_STS_RXBE_RO) {
			continue;
		}
...
int espi_hc_emu_xfr(const uint8_t *msg, size_t msglen, uint8_t *response, size_t resplen)
{
	...
	qr->LDMA_RX_DESCR_BM = 0;
	qr->LDMA_RX_DESCR_BM = 0;
	qr->CTRL = (MCHP_QMSPI_C_IFM_1X | MCHP_QMSPI_C_DESCR_EN);
	qr->EXE = MCHP_QMSPI_EXE_CLR_FIFOS;
	qr->STS = UINT32_MAX;

	/* build message transmit descriptor */
	descr = (MCHP_QMSPI_C_IFM_1X | MCHP_QMSPI_C_TX_DATA | MCHP_QMSPI_C_TX_LDMA_CH0 |
		 MCHP_QMSPI_C_XFR_UNITS_1 | MCHP_QMSPI_C_NEXT_DESCR(1) |
		 ((msglen & 0x7fffu) << MCHP_QMSPI_C_XFR_NUNITS_POS));
	qr->DESCR[0] = descr;

	/* build TAR descriptor. Generate 2 clocks with tri-stated I/O lines */
	descr = (MCHP_QMSPI_C_IFM_1X | MCHP_QMSPI_C_XFR_UNITS_BITS | MCHP_QMSPI_C_NEXT_DESCR(2) |
		 (2u << MCHP_QMSPI_C_XFR_NUNITS_POS));
	qr->DESCR[1] = descr;

	/* build receive response descriptor */
	descr = (MCHP_QMSPI_C_IFM_1X | MCHP_QMSPI_C_RX_EN | MCHP_QMSPI_C_RX_LDMA_CH0 |
		 MCHP_QMSPI_C_XFR_UNITS_1 | MCHP_QMSPI_C_NEXT_DESCR(3) |
		 ((resplen & 0x7fffu) << MCHP_QMSPI_C_XFR_NUNITS_POS) | MCHP_QMSPI_C_DESCR_LAST |
		 MCHP_QMSPI_C_CLOSE);
	qr->DESCR[2] = descr;

	/* Local DMA TX channel 0 to load msg into TX FIFO */
	qr->LDTX[0].LEN = msglen;
	qr->LDTX[0].MSTART = (uint32_t)msg;
	qr->LDTX[0].CTRL = MCHP_QMSPI_LDC_EN | MCHP_QMSPI_LDC_INCR_EN | MCHP_QMSPI_LDC_ASZ_1;

	/* Local DMA RX channel 0 to read response from RX FIFO and store in resp */
	qr->LDRX[0].LEN = resplen;
	qr->LDRX[0].MSTART = (uint32_t)response;
	qr->LDRX[0].CTRL = MCHP_QMSPI_LDC_EN | MCHP_QMSPI_LDC_INCR_EN | MCHP_QMSPI_LDC_ASZ_1;

	/* Set Local DMA TX enable for descr[0] and set Local DMA RX enable for descr[2] */
	qr->LDMA_RX_DESCR_BM = BIT(2);
	qr->LDMA_TX_DESCR_BM = BIT(0);
...
static void init_vw(struct espi_hc_vw *vw, size_t max_vw, const struct espi_hc_vw *init_vw,
		    size_t max_init_vw)
{
 ...
 * 7                 CRC-8
 */
int espi_hc_emu_get_config(struct espi_hc_context *hc, uint16_t config_offset, uint32_t *config,
			   uint16_t *cmd_status)
{
	uint32_t cfg = 0u;
	int ret = 0;
	...
	if (rsp_crc8 != ersp->buf[7]) {
		LOG_ERR("GET_CONFIGURATION: CRC error: expected=0x%02x calc=0x%02x", rsp_crc8,
			ersp->buf[7]);
		*cmd_status = 0xffffu;
		return -EIO;
	}
 ...
 *
 */
int espi_hc_emu_set_config(struct espi_hc_context *hc, uint16_t config_offset, uint32_t config,
			   uint16_t *cmd_status)
{
	int ret = 0;
	uint16_t status = 0;
	...
	if (rsp_crc != rbuf[3]) {
		LOG_ERR("SET_CONFIGURATION: rsp_crc=0x%02x crc=0x%02x", rsp_crc, rbuf[3]);
		*cmd_status = 0xffffu;
		return -EIO;
	}
...
int espi_hc_ctx_emu_set_config(struct espi_hc_context *hc, uint16_t cfg_offset, uint32_t new_config)
{
	int ret;
	uint16_t estatus;
...
int espi_hc_ctx_emu_put_vws(struct espi_hc_context *hc, struct espi_hc_vw *vws, uint8_t nvws)
{
	...
	if (rsp_crc8 != rspbuf[3]) {
		LOG_ERR("PUT_VW: rsp_crc=0x%02x crc=0x%02x", rsp_crc8, rspbuf[3]);
		return -EIO;
	}
			    ...
			    uint8_t *nrecv)
{
	...
	hc->pkt_status =
		(((uint16_t)ersp->buf[rsplen - 2u] << 8) | (uint16_t)ersp->buf[rsplen - 3u]);
	...
	for (n = 0; n < nvw; n++) {
		...
		LOG_INF("  VW group: HI=0x%02x Levels=0x%02x Valid=0x%02x", vws->host_index,
			vws->levels, vws->valid);
		vwb++;
		vws++;
	}
					...
					uint32_t flags)
{
	...
	if (rsp_crc8 != rspbuf[3]) {
		LOG_ERR("PUT_VW MHI: rsp_crc=0x%02x crc=0x%02x", rsp_crc8, rspbuf[3]);
		return -EIO;
	}
 ...
 */
int espi_hc_emu_put_iowr(struct espi_hc_context *hc, uint32_t io_addr_len, uint32_t data,
			 uint16_t *cmd_status)
{
	int ret = 0;
	uint16_t status = 0;
	...
	if (rsp_crc != rbuf[n + 3u]) {
		LOG_ERR("PUT_IOWR: rsp_crc=0x%02x crc=0x%02x", rsp_crc, rbuf[n + 3u]);
		*cmd_status = 0xffffu;
		return -EIO;
	}
...
int espi_hc_emu_put_iord(struct espi_hc_context *hc, uint32_t io_addr_len, uint32_t *data,
			 uint16_t *cmd_status)
{
	...
	if (rsp_crc8 != ersp->buf[(pktlen - 1u)]) {
		LOG_ERR("PUT_IORD: CRC error: expected=0x%02x calc=0x%02x", rsp_crc8,
			ersp->buf[(pktlen - 1u)]);
		*cmd_status = 0xffffu;
		return -EIO;
	}
