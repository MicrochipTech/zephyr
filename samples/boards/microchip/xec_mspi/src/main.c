...
#define MSPI_QMSPI_FLAG 0
#define MSPI_GPSPI_FLAG BIT(0)

#define APP_FLASH_GPSPI_INFO2(fnid, nid)                                                           \
	{                                                                                          \
		.flags = MSPI_GPSPI_FLAG,                                                          \
...
#define APP_FLASH_QMSPI_INFO2(fnid, nid)                                                           \
	{                                                                                          \
		.flags = MSPI_QMSPI_FLAG,                                                          \
		.flash_dev = DEVICE_DT_GET(fnid),                                                  \
		.mspi_dtspec = &app_mspi_dt_##nid,                                                 \
		.mspi_fdid = MSPI_DEVICE_ID_DT(fnid),                                              \
		.mspi_fdcfg = MSPI_DEVICE_CONFIG_DT(fnid),                                         \
...
#define APP_FLASH_GPSPI_INFO(nid)                                                                  \
	DT_FOREACH_CHILD_STATUS_OKAY_VARGS(nid, APP_FLASH_GPSPI_INFO2, nid)

#define APP_FLASH_QMSPI_INFO(nid)                                                                  \
	DT_FOREACH_CHILD_STATUS_OKAY_VARGS(nid, APP_FLASH_QMSPI_INFO2, nid)

static const struct app_flash_info app_flashes[] = {
	DT_FOREACH_STATUS_OKAY(microchip_xec_gpspi_v2_mspi_controller, APP_FLASH_GPSPI_INFO)
		DT_FOREACH_STATUS_OKAY(microchip_xec_qmspi_mspi_controller, APP_FLASH_QMSPI_INFO)};
...
int main(void)
{
	...
	LOG_INF("MEC_Assy6941 board MSPI sample %s", __func__);
