...
#define XFR_BUF_LEN 1024U
static uint8_t xfrbuf[XFR_BUF_LEN];
...
int main(void)
{
	...
	LOG_INF("I2C driver: %s", DRV_LABEL);

	memset(i2c_state_cap_buf, 0, sizeof(i2c_state_cap_buf));
	memset(xfrbuf, 0, sizeof(xfrbuf));
	...
	LOG_INF("Loop back self test");
	rc = loopback_self_test();
	if (rc != 0) {
		LOG_WRN("loopback self-test FAILED (%d); continuing to deep-sleep demo", rc);
	}

	LOG_INF("Write to non-existent device for checking DMA block CLK_REQ");
	rc = i2c_write(smb0_port0, (const uint8_t *)xfrbuf, XFR_BUF_LEN, 0x23);
	LOG_INF("I2C write to non-existent device returned (%d)", rc);

	k_thread_start(app_task_sensor);

#if DT_NODE_HAS_STATUS_OKAY(DT_NODELABEL(i2c_targ_038))
#if defined(CONFIG_PM) || defined(CONFIG_PM_DEVICE)
	while (1) {
		loop_count++;
		LOG_INF("Deep sleep loop %llu", loop_count);
