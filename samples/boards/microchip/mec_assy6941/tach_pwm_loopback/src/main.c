...
#define TACH_ASSERT_EDGES(node_id)                                                                 \
	BUILD_ASSERT((DT_PROP(node_id, tach_edges) == 2) || (DT_PROP(node_id, tach_edges) == 3) || \
...
#define TACH_INFO_ENTRY(node_id)                                                                   \
	{                                                                                          \
		...
		.window_half_periods = TACH_WINDOW_HALF_PERIODS(DT_PROP(node_id, tach_edges)),     \
...
static void report(void)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(tachs); i++) {
		printk("%-14s %5u  %12u  %8u  %5u  %5u  %6u.%03u\n", tachs[i].dev->name,
		       tachs[i].edges, tachs[i].window_half_periods, RAW_READINGS, stats[i].min,
		       stats[i].max, stats[i].mean_milli / MILLI, stats[i].mean_milli % MILLI);
	...
	for (size_t a = 0; a < ARRAY_SIZE(tachs); a++) {
		for (size_t b = 0; b < ARRAY_SIZE(tachs); b++) {
			...
			if (tachs[b].window_half_periods != (2U * tachs[a].window_half_periods)) {
				continue;
			}

			offset = (2 * (int64_t)stats[a].mean_milli) - (int64_t)stats[b].mean_milli;

			printk("%-14s %-14s %6u %6u   ", tachs[a].dev->name, tachs[b].dev->name,
			       tachs[a].window_half_periods, tachs[b].window_half_periods);
	...
	for (size_t i = 0; i < ARRAY_SIZE(tachs); i++) {
		...
		freq_milli = (((uint64_t)tachs[i].clock_hz * MILLI * MILLI) + (period_milli / 2U)) /
			     period_milli;
...
#define ASSERT_PERIOD_CLOCKS(k)                                                                    \
	BUILD_ASSERT((((k) % 2U) == 0U) && ((1000000U % (k)) == 0U) && ((4U * (k)) < 0xffffU),     \
