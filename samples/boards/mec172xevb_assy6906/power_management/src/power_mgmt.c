...
void test_config(void)
{
	...
	GPIO_REGS->CTRL[MCHP_GPIO_0161_ID] = 0x10240U;
	GPIO_REGS->CTRL[MCHP_GPIO_0162_ID] = 0x10240U;

	GPIO_REGS->CTRL[MCHP_GPIO_0060_ID] = 0x2240U;
	r = GPIO_REGS->CTRL[MCHP_GPIO_0060_ID];
	LOG_INF("test_config: GPIO060.Ctrl = 0x%08x", r);
	r = PCR_REGS->SLP_EN[2];
	LOG_INF("test_config: PCR SLP_EN2 = 0x%08x", r);
}
...
/* DEBUG */
static void pr_slp_debug(void)
{
	for (int i = 0; i < 6; i++) {
		
		LOG_INF("Sleep Debug VBATMEM[%d] = 0x%08x", i, VBATM_REGS->MEM.u32[i]);
	}
}
