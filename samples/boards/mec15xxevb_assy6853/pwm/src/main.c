...
static const struct device *pwm_inst1_dev = DEVICE_DT_GET(DT_NODELABEL(pwm1));

#define PWM_INST0_NODE DT_NODELABEL(pwm0)
#define PWM_INST0_BASE_ADDR DT_REG_ADDR(PWM_INST0_NODE)
#define PWM_INST1_NODE DT_NODELABEL(pwm1)
...
int pwm_test(void)
{
	...
	if (ret) {
		LOG_ERR("PWM Instance 0: Set cycles API error %d", ret);
		return ret;
	}

        LOG_INF("Try PWM Instance 1 Get cycles per second");
        pwm_reported_cycles_per_sec = 0U;
        ...
        LOG_INF("PWM Inst 1: Get cycles per sec = %llu", pwm_reported_cycles_per_sec);
        print_pwm_regs(PWM_INST1_BASE_ADDR);

        pwm_period_cycles = 50000;
        pwm_pulse_width_cycles = 25000;
        pwm_flags = 0;
