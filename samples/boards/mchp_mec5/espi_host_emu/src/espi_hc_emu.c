...
{
	mm_reg_t qb = (mm_reg_t)DT_REG_ADDR(DT_NODELABEL(qspi0));
	uint32_t r = 0;
	int ret;
...
static void pr_qspi_regs(mm_reg_t qb)
{
	LOG_INF("QSPI.Mode = 0x%08x", sys_read32(qb + XEC_QSPI_MODE_OFS));
	LOG_INF("QSPI.Ctrl = 0x%08x", sys_read32(qb + XEC_QSPI_CR_OFS));
	LOG_INF("QSPI.Status = 0x%08x", sys_read32(qb + XEC_QSPI_SR_OFS));
	LOG_INF("QSPI.Intr_en = 0x%08x", sys_read32(qb + XEC_QSPI_IER_OFS));
	for (uint32_t n = 0; n < 4u; n++) {
		LOG_INF("QSPI.Descrs[%u] = 0x%08x", n, sys_read32(qb + XEC_QSPI_DESCR_OFS(n)));
	}
	LOG_INF("QSPI.LDMA_RXEN = 0x%08x", sys_read32(qb + XEC_QSPI_LDMA_RX_EN_OFS));
	LOG_INF("QSPI.LDMA_TXEN = 0x%08x", sys_read32(qb + XEC_QSPI_LDMA_TX_EN_OFS));
	LOG_INF("QSPI.RX_LDMA_CH0.CTRL = 0x%08x",
		sys_read32(qb + XEC_QSPI_LDMA_CHX_CR_OFS(XEC_QSPI_LDMA_RX_CH0)));
	LOG_INF("QSPI.RX_LDMA_CH0.MEM_ADDR = 0x%08x",
		sys_read32(qb + XEC_QSPI_LDMA_CHX_SA_OFS(XEC_QSPI_LDMA_RX_CH0)));
	LOG_INF("QSPI.RX_LDMA_CH0.LEN = 0x%08x",
		sys_read32(qb + XEC_QSPI_LDMA_CHX_LR_OFS(XEC_QSPI_LDMA_RX_CH0)));
	LOG_INF("QSPI.TX_LDMA_CH0.CTRL = 0x%08x",
		sys_read32(qb + XEC_QSPI_LDMA_CHX_CR_OFS(XEC_QSPI_LDMA_TX_CH0)));
	LOG_INF("QSPI.TX_LDMA_CH0.MEM_ADDR = 0x%08x",
		sys_read32(qb + XEC_QSPI_LDMA_CHX_SA_OFS(XEC_QSPI_LDMA_TX_CH0)));
	LOG_INF("QSPI.TX_LDMA_CH0.LEN = 0x%08x",
		sys_read32(qb + XEC_QSPI_LDMA_CHX_LR_OFS(XEC_QSPI_LDMA_TX_CH0)));
}
#else
static void pr_qspi_regs(mm_reg_t qb) { ARG_UNUSED(qb); }
#endif

static int qspi_is_hw_error(uint32_t status)
{
	if (status & QSPI_EMU_ALL_ERRORS) {
		return -EIO;
	}
...
int espi_hc_close(void)
{
	...
	if (sys_test_bit(qb + XEC_QSPI_SR_OFS, XEC_QSPI_SR_CS_ASSERTED_POS) != 0) {
		...
		while (sys_test_bit(qb + XEC_QSPI_SR_OFS, XEC_QSPI_SR_CS_ASSERTED_POS) != 0) {
		}
	}
...
int espi_hc_fd_tx_fifo(const uint8_t *msg, size_t msglen, uint32_t flags)
{
	...
	status = sys_read32(qb + XEC_QSPI_SR_OFS);
	if (flags & BIT(0)) {
		...
		sys_write32(0, qb + XEC_QSPI_LDMA_CHX_CR_OFS(XEC_QSPI_LDMA_RX_CH0));
		sys_write32(0, qb + XEC_QSPI_LDMA_CHX_CR_OFS(XEC_QSPI_LDMA_TX_CH0));
	...
	sys_write32(UINT32_MAX, qb + XEC_QSPI_SR_OFS);
	ctrl = XEC_QSPI_CR_IFM_SET(XEC_QSPI_CR_IFM_FD)
	       | XEC_QSPI_CR_TXM_SET(XEC_QSPI_CR_TXM_DATA)
	       | XEC_QSPI_CR_QUNIT_SET(XEC_QSPI_CR_QUNIT_1B)
	       | XEC_QSPI_CR_NQUNITS_SET(msglen);
	if (flags & BIT(1)) {
		ctrl |= BIT(XEC_QSPI_CR_CLOSE_EN_POS);
	}
	sys_write32(ctrl, qb + XEC_QSPI_CR_OFS); /* write all bits at once */

	/* preload TX FIFO up to QSPI HW FIFO length */
	while (ntx) {
		status = sys_read32(qb + XEC_QSPI_SR_OFS);
		if (status & BIT(XEC_QSPI_SR_TXB_FULL_POS)) {
			break;
		}
		sys_write8(*pdata, qb + XEC_QSPI_TXB_OFS);
		pdata++;
		ntx--;
	}
	 ...
	 */
	status = sys_read32(qb + XEC_QSPI_SR_OFS);
	while (!(status & BIT(XEC_QSPI_SR_TXB_EMPTY_POS))) {
		status = sys_read32(qb + XEC_QSPI_SR_OFS);
	}
	 ...
	 */
	status = sys_read32(qb + XEC_QSPI_SR_OFS);
	while (!(status & BIT(XEC_QSPI_SR_XFR_DONE_POS))) {
		status = sys_read32(qb + XEC_QSPI_SR_OFS);
	}
...
int espi_hc_fd_tar(void)
{
	...
	sys_write32(UINT32_MAX, qb + XEC_QSPI_SR_OFS);
	ctrl = XEC_QSPI_CR_IFM_SET(XEC_QSPI_CR_IFM_FD)
	       | XEC_QSPI_CR_TXM_SET(XEC_QSPI_CR_TXM_DIS)
	       | XEC_QSPI_CR_QUNIT_SET(XEC_QSPI_CR_QUNIT_BITS)
	       | XEC_QSPI_CR_NQUNITS_SET(2u);
	sys_write32(ctrl, qb + XEC_QSPI_CR_OFS); /* write all bits at once */
...
int espi_hc_fd_rx_fifo(uint8_t *msg, size_t msglen, uint32_t flags)
{
	...
	sys_write32(UINT32_MAX, qb + XEC_QSPI_SR_OFS);
	ctrl = XEC_QSPI_CR_IFM_SET(XEC_QSPI_CR_IFM_FD) | BIT(XEC_QSPI_CR_RX_EN_POS)
	       | XEC_QSPI_CR_QUNIT_SET(XEC_QSPI_CR_QUNIT_1B)
	       | XEC_QSPI_CR_NQUNITS_SET(msglen);
	if (flags & BIT(1)) {
		ctrl |= BIT(XEC_QSPI_CR_CLOSE_EN_POS);
	...
	while (nrx) {
		status = sys_read32(qb + XEC_QSPI_SR_OFS);
		if (status & BIT(XEC_QSPI_SR_RXB_EMPTY_POS)) {
			continue;
		}
		*pdata = sys_read8(qb + XEC_QSPI_RXB_OFS);
		pdata++;
		nrx--;
	}
...
int espi_hc_emu_xfr(const uint8_t *msg, size_t msglen, uint8_t *response, size_t resplen)
{
	...
	sys_write32(0, qb + XEC_QSPI_LDMA_RX_EN_OFS);
	sys_write32(0, qb + XEC_QSPI_LDMA_TX_EN_OFS);
	sys_write32(BIT(XEC_QSPI_CR_DESCR_EN_POS) | XEC_QSPI_CR_IFM_SET(XEC_QSPI_CR_IFM_FD),
		    qb + XEC_QSPI_CR_OFS);
	sys_write32(BIT(XEC_QSPI_EXE_CLR_FIFOS_POS), qb + XEC_QSPI_EXE_OFS);
	sys_write32(UINT32_MAX, qb + XEC_QSPI_SR_OFS);

	/* build message transmit descriptor */
	descr = XEC_QSPI_CR_IFM_SET(XEC_QSPI_CR_IFM_FD)
		| XEC_QSPI_CR_TXM_SET(XEC_QSPI_CR_TXM_DATA)
		| XEC_QSPI_CR_TXDMA_SET(XEC_QSPI_CR_LDMA_CH0)
		| XEC_QSPI_CR_QUNIT_SET(XEC_QSPI_CR_QUNIT_1B)
		| XEC_QSPI_DR_ND_SET(1u)
		| XEC_QSPI_CR_NQUNITS_SET(msglen);
	sys_write32(descr, qb + XEC_QSPI_DESCR_OFS(0));

	/* build TAR descriptor. Generate 2 clocks with tri-stated I/O lines */
	descr = XEC_QSPI_CR_IFM_SET(XEC_QSPI_CR_IFM_FD)
		| XEC_QSPI_CR_QUNIT_SET(XEC_QSPI_CR_QUNIT_BITS)
		| XEC_QSPI_DR_ND_SET(2u)
		| XEC_QSPI_CR_NQUNITS_SET(2u);
	sys_write32(descr, qb + XEC_QSPI_DESCR_OFS(1));

	/* build receive response descriptor */
	descr = XEC_QSPI_CR_IFM_SET(XEC_QSPI_CR_IFM_FD)
		| BIT(XEC_QSPI_CR_RX_EN_POS)
		| XEC_QSPI_CR_RXDMA_SET(XEC_QSPI_CR_LDMA_CH0)
		| XEC_QSPI_CR_QUNIT_SET(XEC_QSPI_CR_QUNIT_1B)
		| XEC_QSPI_CR_NQUNITS_SET(resplen)
		| BIT(XEC_QSPI_CR_CLOSE_EN_POS) | BIT(XEC_QSPI_DR_LD_POS);
	sys_write32(descr, qb + XEC_QSPI_DESCR_OFS(2));

	/* Local DMA TX channel 0 to load msg into TX FIFO */
	sys_write32(msglen, qb + XEC_QSPI_LDMA_CHX_LR_OFS(XEC_QSPI_LDMA_TX_CH0));
	sys_write32((uint32_t)msg, qb + XEC_QSPI_LDMA_CHX_SA_OFS(XEC_QSPI_LDMA_TX_CH0));
	sys_write32(XEC_QSPI_LDMA_CHX_CR_SZ_SET(XEC_QSPI_LDMA_CHX_CR_SZ_1B)
		    | BIT(XEC_QSPI_LDMA_CHX_CR_EN_POS) | BIT(XEC_QSPI_LDMA_CHX_CR_INCRA_POS),
		    qb + XEC_QSPI_LDMA_CHX_CR_OFS(XEC_QSPI_LDMA_TX_CH0));

	/* Local DMA RX channel 0 to read response from RX FIFO and store in resp */
	sys_write32(resplen, qb + XEC_QSPI_LDMA_CHX_LR_OFS(XEC_QSPI_LDMA_RX_CH0));
	sys_write32((uint32_t)response, qb + XEC_QSPI_LDMA_CHX_SA_OFS(XEC_QSPI_LDMA_RX_CH0));
	sys_write32(XEC_QSPI_LDMA_CHX_CR_SZ_SET(XEC_QSPI_LDMA_CHX_CR_SZ_1B)
		    | BIT(XEC_QSPI_LDMA_CHX_CR_EN_POS) | BIT(XEC_QSPI_LDMA_CHX_CR_INCRA_POS),
		    qb + XEC_QSPI_LDMA_CHX_CR_OFS(XEC_QSPI_LDMA_RX_CH0));

	/* Set Local DMA TX enable for descrs[0] and set Local DMA RX enable for descrs[2] */
	sys_write32(BIT(2), qb + XEC_QSPI_LDMA_RX_EN_OFS);
