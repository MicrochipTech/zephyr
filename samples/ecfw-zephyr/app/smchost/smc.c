...
static const bool acpi_tbl_attr[256] = {
	...
	/* [242 / F2] uint8_t unused1; */
	ACPI_ATTR_READ_ONLY,

	/* [243 / F3] uint8_t unused2; */
	ACPI_ATTR_READ_ONLY,
...
void smc_update_thermal_sensor(enum acpi_thrm_sens_idx idx, int16_t temp)
{
	...
	case ACPI_THRM_SEN_PCH:
		g_acpi_tbl.acpi_sen1_lsb = (uint8_t) temp;
		g_acpi_tbl.acpi_sen1_msb = (uint8_t) (temp >> 8);
		break;

	case ACPI_THRM_SEN_SKIN:
		g_acpi_tbl.acpi_sen2_lsb = (uint8_t) temp;
		g_acpi_tbl.acpi_sen2_msb = (uint8_t) (temp >> 8);
		break;

	case ACPI_THRM_SEN_AMBIENT:
		g_acpi_tbl.acpi_sen3_lsb = (uint8_t) temp;
		g_acpi_tbl.acpi_sen3_msb = (uint8_t) (temp >> 8);
		break;

	case ACPI_THRM_SEN_VR:
		g_acpi_tbl.acpi_sen4_lsb = (uint8_t) temp;
		g_acpi_tbl.acpi_sen4_msb = (uint8_t) (temp >> 8);
		break;

	case ACPI_THRM_SEN_DDR:
		g_acpi_tbl.acpi_sen5_lsb = (uint8_t) temp;
		g_acpi_tbl.acpi_sen5_msb = (uint8_t) (temp >> 8);
		break;
...
void smc_update_fan_tach(uint8_t fan_idx, uint16_t rpm)
{
	switch (fan_idx) {
	case FAN_CPU:
		g_acpi_tbl.acpi_cpu_fan_rpm_l = (uint8_t) rpm;
		g_acpi_tbl.acpi_cpu_fan_rpm_h = (uint8_t) (rpm >> 8);
		break;
...
void smc_update_therm_trip_status(uint16_t status)
{
	if (status != ((g_acpi_tbl.acpi_therm_snsr_sts_msb << 8)
		...
		g_acpi_tbl.acpi_therm_snsr_sts_lsb = (uint8_t) status;
