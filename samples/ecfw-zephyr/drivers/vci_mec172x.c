...
void vci_enable(void)
{
	...
	VCI_REGS->LATCH_RST |= (MCHP_VCI_LER_IN0 | MCHP_VCI_LER_IN1);
	/* Enable latches on VCI_IN0 so that assertions will be latched
	 * until next reset
	 */
	VCI_REGS->LATCH_EN |= (MCHP_VCI_LER_IN0 | MCHP_VCI_LER_IN1);
...
#if 0 /* !!!MCHP TODO VCI.Config bit[7] does not exist on MEC172x!!! */
