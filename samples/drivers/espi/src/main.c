...
#if CONFIG_ESPI_SAF
			if (event.evt_data == 1) {
				/* TODO: Hack to activate SAF
				 * The callback is from the ESPI driver
				 * so struct device points to ESPI not SAF.
				 */
				int rc = espi_saf_activate(espi_saf_dev);
				LOG_INF("SAF Activate(0=success) = %d", rc);
			}
#endif
