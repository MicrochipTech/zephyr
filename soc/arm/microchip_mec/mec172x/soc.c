...
static int soc_pcr_init(void)
{
	...
	for (int i = 0; i < MCHP_MAX_PCR_SCR_REGS; i++) {
		PCR_REGS->SLP_EN[i] = 0U;
	}

	PCR_REGS->SLP_EN[2] |= BIT(MCHP_PCR2_UART0_POS)
		| BIT(MCHP_PCR2_UART1_POS);
	PCR_REGS->SLP_EN[3] = MCHP_PCR3_CRYPTO_MASK;
...
static int soc_ecia_init(void)
{
	...
	ECIA_AR_REGS->BLK_EN_CLR = 0xFFFFFFFFul;
	/* gate on GIRQ's that are aggregated only */
	ECIA_AR_REGS->BLK_EN_SET = MCHP_ECIA_AGGR_BITMAP;

	/* Clear all GIRQn source enables */
	for (n = 0; n < MCHP_GIRQ_IDX_MAX; n++) {
		ECIA_AR_REGS->GIRQ[n].EN_CLR = 0xFFFFFFFFul;
	}
...
{
	if (VBATR_REGS->CLK32_SRC & MCHP_VBATR_CS_SO_EN) {
		return true;
	}
...
{
	VBATR_REGS->CLK32_SRC |= MCHP_VBATR_CS_SO_EN;
	spin_delay(delay_cnt);
}

static int enable_32k_pin(uint32_t wait_cnt)
{
	GPIO_REGS->CTRL[MCHP_GPIO_0165_ID] = PIN_32KHZ_IN_MODE;

	do {
		if (PCR_REGS->PWR_RST_STS & MCHP_PCR_PRS_32K_ACTIVE_RO) {
			PCR_REGS->CLK32K_SRC_VTR = MCHP_PCR_VTR_32K_SRC_SILOSC;
			return 0;
		}
	} while (wait_cnt--);
...
{
	uint32_t vbcs = VBATR_REGS->CLK32_SRC;
	...
	VBATR_REGS->CLK32_SRC = vbcs | MCHP_VBATR_CS_XTAL_CNTR_DG;
	VBATR_REGS->CLK32_SRC |= MCHP_VBATR_CS_SO_EN;
	spin_delay(CLK32K_XTAL_WAIT);
	VBATR_REGS->CLK32_SRC |= MCHP_VBATR_CS_XTAL_DHC;
...
{
	PCR_REGS->CNT32K_CTRL = 0U;
	PCR_REGS->CLK32K_MON_IEN = 0U;
	PCR_REGS->CLK32K_MON_ISTS = MCHP_PCR_CLK32M_ISTS_MASK;
#if 0
	PCR_REGS->CLK32K_MON_IEN =
		MCHP_PCR_CLK32M_IEN_PULSE_RDY | MCHP_PCR_CLK32M_IEN_PASS_PER |
		MCHP_PCR_CLK32M_IEN_PASS_DC | MCHP_PCR_CLK32M_IEN_FAIL |
		MCHP_PCR_CLK32M_IEN_STALL | MCHP_PCR_CLK32M_IEN_VALID;
#endif

	PCR_REGS->CNT32K_PER_MIN = CNT32K_TMIN;
	PCR_REGS->CNT32K_PER_MAX = CNT32K_TMAX;
	PCR_REGS->CNT32K_DV_MAX = CNT32K_DUTY_MAX;
	PCR_REGS->CNT32K_VALID_MIN = CNT32K_VAL_MIN;

	PCR_REGS->CNT32K_CTRL =
		MCHP_PCR_CLK32M_CTRL_PER_EN | MCHP_PCR_CLK32M_CTRL_DC_EN |
		MCHP_PCR_CLK32M_CTRL_VAL_EN | MCHP_PCR_CLK32M_CTRL_CLR_CNT;

	uint32_t status = PCR_REGS->CLK32K_MON_ISTS;

	while (status == 0) {
		status = PCR_REGS->CLK32K_MON_ISTS;
	}
			       ...
			       uint32_t flags)
{
	if (dest == CLK32K_DEST_PLL) {
		...
		case CLK32K_SRC_CRYSTAL:
			...
			break;
		case CLK32K_SRC_PIN:
			PCR_REGS->CLK32K_SRC_VTR = MCHP_PCR_VTR_32K_SRC_PIN;
			break;
		...
		uint32_t vbcs = VBATR_REGS->CLK32_SRC &
				~(MCHP_VBATR_CS_PCS_MSK);

		switch (src) {
		case CLK32K_SRC_SIL_OSC:
			VBATR_REGS->CLK32_SRC = vbcs |
						MCHP_VBATR_CS_PCS_VTR_VBAT_SO;
			break;
		case CLK32K_SRC_CRYSTAL:
			VBATR_REGS->CLK32_SRC =
				vbcs | MCHP_VBATR_CS_PCS_VTR_VBAT_XTAL;
			break;
		case CLK32K_SRC_PIN:
...
{
	while (!(PCR_REGS->OSC_ID & MCHP_PCR_OSC_ID_PLL_LOCK)) {
		if (wait_cnt == 0) {
			return -1;
		}
		--wait_cnt;
...
static int soc_init(const struct device *dev)
{
	...
	int rc;
	enum clk32k_src pll_clk_src, periph_clk_src;
	...
	clk32_flags = 0U;
	pll_clk_src = CLK32K_SRC_SIL_OSC;
	periph_clk_src = CLK32K_SRC_SIL_OSC;

	if (DT_NODE_HAS_PROP(DT_NODELABEL(CLOCK), pll_32k_src)) {
		pll_clk_src = DT_PROP(DT_ALIAS(clock32k), pll_32k_src);
	}

	if (DT_NODE_HAS_PROP(DT_NODELABEL(CLOCK), periph_32k_src)) {
		periph_clk_src = DT_PROP(DT_ALIAS(clock32k), periph_32k_src);
	}

	rc = soc_clk32_init(pll_clk_src, periph_clk_src, clk32_flags);
	__ASSERT(rc == 0, "SoC: PLL and 32 KHz clock initialization failed");

	/*
	 * On HW reset PCR Processor Clock Divider = 4 for 96/4 = 24 MHz.
	 ...
	 */
	BUILD_ASSERT(CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC <= MCHP_EC_CLOCK_INPUT_HZ);

#ifdef CONFIG_RTOS_TIMER
	PCR_REGS->PROC_CLK_CTRL = CONFIG_SOC_MEC172X_PROC_CLK_DIV;
#else
	PCR_REGS->PROC_CLK_CTRL = 
		(MCHP_EC_CLOCK_INPUT_HZ / CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC);
#endif
