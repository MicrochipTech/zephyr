...
{
	return (uint32_t)(MCHP_EC_CLOCK_INPUT_HZ) / PCR_REGS->PROC_CLK_CTRL;
}
...
void soc_set_core_clock_div(uint8_t clkdiv)
{
	...
	__NOP();
	__NOP();
	PCR_REGS->PROC_CLK_CTRL = (uint32_t)clkdiv;
	__NOP();
	__NOP();
	__NOP();
	__NOP();
...
static int soc_init(const struct device *dev)
{
	...
	pll_clk_src = DT_PROP_OR(PCR, pll_32k_src, CLK32K_SRC_SIL_OSC);
	periph_clk_src =
		DT_PROP_OR(PCR, periph_32k_src, CLK32K_SRC_SIL_OSC);

	rc = soc_clk32_init(pll_clk_src, periph_clk_src, clk32_flags);
	__ASSERT(rc == 0, "SoC: PLL and 32 KHz clock initialization failed");

	soc_set_core_clock_div(DT_PROP_LEN_OR(PCR, cpu_clk_div, 1U));
