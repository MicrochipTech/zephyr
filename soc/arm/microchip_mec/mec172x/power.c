 ...
 * from being shut off.
 */
#ifdef CONFIG_MCHP_XEC_BR_TIMER
extern void mchp_clock_pm_set_wake_timeout(uint32_t wake_us);
extern void mchp_clock_pm_deep_sleep_adj(uint32_t adj32k_ticks);
#endif

/*
 * NOTE: Zephyr has masked interrupts using BASEPRI before calling PM subsystem.
 */
