...
void soc_lite_sleep_disable(void)
{
...
void soc_deep_sleep_wait_clk_idle(void)
{
	...
	VBATM_REGS->MEM.u32[5] = ECS_REGS->SLP_STS_MIRROR;
#endif
}
...
#else
	PCR_REGS->SLP_EN[2] |= 
		(BIT(MCHP_PCR2_UART0_POS) | BIT(MCHP_PCR2_UART1_POS));
#endif
}
#endif
...
/* !!! I2C Control and Status are different registers !!! 
 * In Controller only mode, status bit[6] (SAD) will not be set.
 * Control register bit[6] is ESO which must be set to enable driving
 * of output data on SDA.
 * TODO - detect if controller is in use so we don't enable an unused
 * controller on restore.
 */
#ifdef CONFIG_I2C
	/* SMBUS */
...
static void deep_sleep_restore_blocks(void)
{
	...
	/* Port 80 TODO do we need to do anything? */
}
